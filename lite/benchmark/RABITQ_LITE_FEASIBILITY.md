...
At cf7a0f08, fixed-count SIFT-10k/100k runs completed 250/60 Update-Remove-Add cycles with exact post-load result equality and no stderr. Median Update/Add P50 increased from 4.118/4.027 ms at 10k to 39.843/39.208 ms at 100k; Remove increased from 0.446 to 4.449 ms. Full-code self Top-1 stayed 1.000, while graph self Top-1 medians were 0.960/0.920. These results establish the next bounded target: replace exhaustive mutation neighbor discovery with the existing filter-first graph traversal while keeping exhaustive full-code selection as a differential quality oracle. The all-adjacency Remove scan remains required for asymmetric correctness unless a separate reverse-edge structure is justified.
