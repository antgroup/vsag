...
public:
     ...
     *          - Batched RANGE_SEARCH is not supported for result search; implementations MUST
     *            reject NumElements > 1, except IVF disable_bucket_scan routing-only mode,
     *            which returns bucket routes rather than range-search results.
     */
