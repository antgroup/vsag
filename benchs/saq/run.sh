...
result_root=${2:-${repo_root}/benchs/saq/results/local}
...
target_recall=${SAQ_TARGET_RECALL:-0.95}
sift_common_recall=${SAQ_SIFT_COMMON_RECALL:-0.85}
gist_common_recall=${SAQ_GIST_COMMON_RECALL:-0.91}
...
cmake --build "${build_dir}" --target eval_performance saq_quantization_benchmark \
    --parallel "${build_jobs}"

eval_binary="${build_dir}/tools/eval/eval_performance"
quant_binary="${build_dir}/tools/saq_quantization_benchmark/saq_quantization_benchmark"
template="${script_dir}/configs/hgraph_l2_4bit.yaml.in"
...
run_dataset() {
    ...
    local filename=$2
    local dim=$3
    local dataset_path="${data_root}/${filename}"
    local output_dir="${result_root}/${name}"
    local config_path="${output_dir}/eval.yaml"
    mkdir -p "${output_dir}"
    ...
    python3 "${script_dir}/generate_config.py" \
        ...
        --dim "${dim}" \
        --index-root "${output_dir}/indexes" \
        --result-json "${output_dir}/eval.json" \
        --output "${config_path}"
    ...
    "${quant_binary}" "${dataset_path}" "${output_dir}/quantization.json" 4 \
        "${train_count}" "${encode_count}" "${quantizer_flags[@]}" \
        2>&1 | tee "${output_dir}/quantization.stdout.txt"
    "${eval_binary}" "${config_path}" 2>&1 | tee "${output_dir}/eval.stdout.txt"
}

{
    ...
    echo "sift_common_recall=${sift_common_recall}"
    echo "gist_common_recall=${gist_common_recall}"
    echo "extra_c_flags=${extra_c_flags}"
    echo "extra_cxx_flags=${extra_cxx_flags}"
    echo "cmake_build_type=Release"
    echo "hgraph_max_degree=32"
    echo "hgraph_ef_construction=300"
    echo "hgraph_threads=16"
    echo "ef_search=40,80,120,200,400,800,1600"
    echo "topk=10"
    echo "search_query_count=10000"
    echo "average_bits_per_dimension=4"
    echo "saq_adjustment_rounds=6"
    echo "saq_use_pca=true"
    echo "saq_random_rotation=true"
    echo "saq_random_rotation_seed=20260825"
    echo "saq_segment_count=0"
...
ldd "${eval_binary}" >> "${environment_path}"

run_dataset "sift1m" "sift-128-euclidean.hdf5" 128
run_dataset "gist1m" "gist-960-euclidean.hdf5" 960
python3 "${script_dir}/summarize.py" "${result_root}" \
    --target-recall "${target_recall}" \
    --common-recall-target "sift1m=${sift_common_recall}" \
    --common-recall-target "gist1m=${gist_common_recall}"
