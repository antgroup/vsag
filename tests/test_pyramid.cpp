...
TEST_CASE("Pyramid ExportCache + ImportCache + Build acceleration smoke test",
    ...
    // ---- (3) preserve SourceID in its dedicated streaming block ----
    std::stringstream index_buf;
    REQUIRE(all_in_one->SerializeStreaming(index_buf).has_value());
    index_buf.seekg(0);
    auto restored = vsag::Factory::CreateIndex("pyramid", param).value();
    REQUIRE(restored->DeserializeStreaming(index_buf).has_value());

    // ---- (4) export cache from the restored index ----
    std::stringstream cache_buf;
    cache_buf.put('x');
    auto export_result = restored->ExportCache(cache_buf);
    REQUIRE(export_result.has_value());
    REQUIRE(cache_buf.tellp() > 0);

    // ---- (5) fresh index, import cache, build again ----
    cache_buf.seekg(1);
