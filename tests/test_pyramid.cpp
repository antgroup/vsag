...
// Appended tests for the Pyramid build-cache (ExportCache/ImportCache) at the
// end of test_pyramid.cpp.
...
MakePyramidCacheDataset(int64_t count,
    ...
    auto dataset = vsag::Dataset::Make();
    auto* raw_vectors = new float[count * dim];
    auto* raw_ids = new int64_t[count];
    auto* raw_paths = new std::string[count];
    auto* raw_source_ids = new std::string[count];

    std::copy(vectors.begin(), vectors.end(), raw_vectors);
    std::copy(ids.begin(), ids.end(), raw_ids);
    std::copy(paths.begin(), paths.end(), raw_paths);
    std::copy(source_ids.begin(), source_ids.end(), raw_source_ids);
...
MakePyramidCacheQuery(int64_t dim, const std::vector<float>& query, const std::string& path)
    -> vsag::DatasetPtr {
    auto dataset = vsag::Dataset::Make();
    auto* raw_query_vec = new float[dim];
    std::copy(query.begin(), query.end(), raw_query_vec);
    auto* raw_paths = new std::string[1];
    raw_paths[0] = path;
    dataset->NumElements(1)->Dim(dim)->Float32Vectors(raw_query_vec)->Paths(raw_paths)->Owner(true);
    return dataset;
}
...
TEST_CASE("Pyramid ExportCache + ImportCache + Build acceleration smoke test",
    ...
    // GRAPH that fullfill_cache() walks.
    ...
    // usable cache after a Build that recorded source_ids.
    const auto* param = R"(
    {
        "dtype": "float32",
        "metric_type": "l2",
    ...
    // exceed index_min_size and become a GRAPH that fullfill_cache() walks.
...
TEST_CASE("Pyramid GetStats reports build cache hit-rate", "[ft][pyramid][cache][pr]") {
    ...
    //       the hit / missed node counts, and miss + hit covers the warm
    //       build's vector count (>= index size, accounting for FLAT nodes).
    constexpr int64_t TEST_DIM = 32;
    constexpr int64_t TEST_COUNT = 200;
