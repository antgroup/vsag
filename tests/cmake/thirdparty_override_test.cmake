...
file (GLOB_RECURSE thirdparty_source_files
      "${VSAG_SOURCE_DIR}/extern/*.cmake"
      "${VSAG_SOURCE_DIR}/extern/*.patch")
set (override_inventory_count 0)
foreach (thirdparty_source_file IN LISTS thirdparty_source_files)
    file (READ "${thirdparty_source_file}" thirdparty_source_content)
    string (TOLOWER "${thirdparty_source_content}" thirdparty_source_content_lower)
    if (thirdparty_source_content_lower MATCHES
            "aliyuncs\\.com|vsagcache\\.oss|maintained by the vsag project")
        message (FATAL_ERROR
                 "Alibaba Cloud OSS cache remains in ${thirdparty_source_file}")
    endif ()
    if (thirdparty_source_content MATCHES "vsag_resolve_thirdparty_override")
        math (EXPR override_inventory_count "${override_inventory_count} + 1")
        if (NOT thirdparty_source_content MATCHES "https://")
            message (FATAL_ERROR
                     "Third-party override has no authoritative upstream URL: "
                     "${thirdparty_source_file}")
        endif ()
    endif ()
endforeach ()
if (NOT override_inventory_count EQUAL 15)
    message (FATAL_ERROR
             "Expected 15 third-party archive overrides, found ${override_inventory_count}")
endif ()
...
assert_matches ("${yaml_cpp_content}"
                ...
                "0.17 yaml-cpp pin")
if (EXISTS "${VSAG_SOURCE_DIR}/extern/httplib/httplib.cmake")
    message (FATAL_ERROR "0.17 must not add httplib support")
endif ()
...
assert_equal ("${actual}" "VSAG_THIRDPARTY_OPENBLAS_0_3_23" "0.17 OpenBLAS pin")
vsag_thirdparty_pinned_variable (YAML_CPP 0.8.0 actual)
assert_equal ("${actual}" "VSAG_THIRDPARTY_YAML_CPP_0_8_0" "0.17 yaml-cpp pin")
