...
main() {
    ...
    float v[] = {1, 1};
    float out[] = {0, 0};
    B::Sgemv(CblasRowMajor, CblasNoTrans, 2, 2, 1, a, 2, v, 1, 0, out, 1);
    check(near(out[0], 3) && near(out[1], 7));
    float product[4] = {};
    B::Sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 2, 2, 2, 1, a, 2, a, 2, 0, product, 2);
    check(near(product[0], 7) && near(product[3], 22));
    float qr[] = {1, 2, 3, 4};
    float tau[2];
    check(B::Sgeqrf(LAPACK_ROW_MAJOR, 2, 2, qr, 2, tau) == 0);
    check(B::Sorgqr(LAPACK_ROW_MAJOR, 2, 2, 2, qr, 2, tau) == 0);
    check(near(qr[0] * qr[0] + qr[2] * qr[2], 1));
    check(near(qr[0] * qr[1] + qr[2] * qr[3], 0));
    float lu[] = {4, 1, 2, 3};
    int32_t pivots[2];
    check(B::Sgetrf(LAPACK_ROW_MAJOR, 2, 2, lu, 2, pivots) == 0);
    check(near(lu[3], 2.5F));
    float symmetric[] = {2, 1, 1, 2};
    float eigenvalues[2];
    check(B::Ssyev(LAPACK_ROW_MAJOR, 'V', 'U', 2, symmetric, 2, eigenvalues) == 0);
    ...
    std::puts("All eight VSAG BLAS/LAPACKE wrappers passed");
}
