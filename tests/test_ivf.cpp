...
TEST_CASE_PERSISTENT_FIXTURE(fixtures::IVFTestIndex,
    ...
    SECTION("batch routing ignores search-only options") {
        std::vector<float> batch(dim * 2);
        std::memcpy(batch.data(), query_vector.data(), dim * sizeof(float));
        std::memcpy(batch.data() + dim, query_vector.data(), dim * sizeof(float));
        auto batch_query = vsag::Dataset::Make();
        batch_query->NumElements(2)->Dim(dim)->Float32Vectors(batch.data())->Owner(false);
        vsag::SearchRequest batch_req;
        batch_req.query_ = batch_query;
        batch_req.mode_ = vsag::SearchMode::RANGE_SEARCH;
        batch_req.topk_ = 1;
        batch_req.params_str_ = route_params;
        auto batch_result = index->SearchWithRequest(batch_req);
        REQUIRE(batch_result.has_value());
        check_bucket_result(batch_result.value(), 2, scan_buckets_count, buckets_count);
    }
...
TEST_CASE("IVF Batch Search Parallel", "[ft][ivf][pr]") {
    ...
    const auto serial_stats =
        serial_result.value()->GetStatistics({"distance_evaluations", "complete"});
    const auto parallel_stats =
        parallel_result.value()->GetStatistics({"distance_evaluations", "complete"});
    REQUIRE(std::stoull(serial_stats[0]) > 0);
    REQUIRE(std::stoull(parallel_stats[0]) > 0);
    REQUIRE(serial_stats[0] == parallel_stats[0]);
    REQUIRE(serial_stats[1] == "true");
    REQUIRE(parallel_stats[1] == "true");
