...
TEST_CASE("DiskANN Statistics Metrics", "[ft][diskann][statistics]") {
    int dim = 128;
    ...
    auto supported_keys = diskann->GetSupportedMetricKeys();
    REQUIRE(!supported_keys.empty());
    
    // Check that expected keys are present
    std::set<std::string> key_set(supported_keys.begin(), supported_keys.end());
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_TIME) > 0);
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_IO) > 0);
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_IO_TIME) > 0);
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_IO_MAX_TIME) > 0);
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_PREREORDER_TIME) > 0);
    REQUIRE(key_set.count(vsag::STATSTIC_KNN_REORDER_TIME) > 0);
    
    // Perform some searches to generate statistics
    nlohmann::json parameters{
        {"diskann", {{"ef_search", ef_search}, {"beam_search", 4}, {"io_limit", 200}, {"use_reorder", true}}}};
    int64_t k = 10;
    for (int i = 0; i < 10; i++) {
        auto query = vsag::Dataset::Make();
        query->NumElements(1)->Dim(dim)->Float32Vectors(data + i * dim)->Owner(false);
    ...
    // Get statistics and parse JSON
    std::string stats_json = diskann->GetStats();
    auto stats = nlohmann::json::parse(stats_json);
    
    // Verify new metrics are present in stats
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_TIME));
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_IO));
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_IO_TIME));
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_IO_MAX_TIME));
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_PREREORDER_TIME));
    REQUIRE(stats.contains(vsag::STATSTIC_KNN_REORDER_TIME));
    
    // Verify metrics have reasonable values
    REQUIRE(stats[vsag::STATSTIC_KNN_TIME].get<float>() > 0);
    REQUIRE(stats[vsag::STATSTIC_KNN_IO].get<float>() >= 0);
    REQUIRE(stats[vsag::STATSTIC_KNN_PREREORDER_TIME].get<float>() >= 0);
    REQUIRE(stats[vsag::STATSTIC_KNN_REORDER_TIME].get<float>() >= 0);
    
    // Max IO time should be >= average IO time
    if (stats[vsag::STATSTIC_KNN_IO].get<float>() > 0) {
        REQUIRE(stats[vsag::STATSTIC_KNN_IO_MAX_TIME].get<float>() >= 
                stats[vsag::STATSTIC_KNN_IO_TIME].get<float>());
    }
