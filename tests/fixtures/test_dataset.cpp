...
float get_sparse_distance(const vsag::SparseVector& vec1, const vsag::SparseVector& vec2) {
...
CalDistanceFloatMetrix(const vsag::DatasetPtr query,
                       ...
                       const std::string& metric_str, const std::string& vector_type = "dense") {
    ...
    for (uint64_t i = 0; i < query_count; ++i) {
        ...
        for (uint64_t j = 0; j < base_count; ++j) {
            ...
            if (vector_type == "dense") {
                ...
                dist = get_sparse_distance(query->GetSparseVectors()[i], base->GetSparseVectors()[j]);
            } else {
...
TestDataset::CreateTestDataset(
    ...
    {
        auto result = CalDistanceFloatMetrix(dataset->query_, dataset->base_, metric_str, vector_type);
        dataset->top_k = 10;
