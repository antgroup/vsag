...
TEST_CASE("FP16/BF16 Conversion Test", "[ut][simd][fp16]") {
    float test_val = 1.5f;
...
TEST_CASE("HGraph with FP16 Test", "[ft][hgraph][fp16]") {
    ...
    if (index->GetNumElements() > 0) {
        ...
        query->NumElements(1)->Dim(TEST_DIM)->Float32Vectors(fp32_data.data())->Owner(false);
...
TEST_CASE("IVF with FP16 Test", "[ft][ivf][fp16]") {
    constexpr int64_t TEST_DIM = 32;
    constexpr int64_t TEST_COUNT = 100;
    ...
    auto index = vsag::Factory::CreateIndex("ivf", param).value();
    ...
    if (index->GetNumElements() > 0) {
        ...
        auto search_param = R"({"ivf": {"nprobe": 10, "scan_buckets_count": 10}})";
        auto search_result = index->KnnSearch(query, 10, search_param);
        REQUIRE(search_result.has_value());
    }
}

TEST_CASE("IVF with BF16 Test", "[ft][ivf][bf16]") {
    constexpr int64_t TEST_DIM = 32;
    constexpr int64_t TEST_COUNT = 100;
