...
constexpr const char* STREAM_MAGIC = "vsagstm0";
...
parse_streaming_index(const std::string& index_path) {
    ...
    if (std::string(magic.data(), 8) != STREAM_MAGIC) {
        throw std::runtime_error(
            "unsupported index format: only streaming magic vsagstm0 is supported");
    }
...
write_html(const ParsedIndex& parsed, const std::string& output_path) {
    ...
    output << "</tbody></table>\n";
    output << "<h2>Metadata JSON</h2><pre>" << escape_html(parsed.metadata_json)
           << "</pre>\n</body></html>\n";
}
