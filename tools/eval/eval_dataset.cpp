...
EvalDataset::Load(const std::string& filename) {
    ...
    bool has_valid_ratio = false;
    {
        ...
        has_feature_ids = datasets.count("train_feature_ids") > 0;
    ...
    if (has_feature_ids) {
        H5::DataSet feature_ids_dataset = file.openDataSet("/train_feature_ids");
        H5::DataSpace feature_ids_dataspace = feature_ids_dataset.getSpace();
        hsize_t dims_out[1];
        feature_ids_dataspace.getSimpleExtentDims(dims_out, NULL);
        if (static_cast<int64_t>(dims_out[0]) != obj->number_of_base_) {
            throw std::runtime_error("train_feature_ids row count does not match train rows");
        }

        auto datatype = feature_ids_dataset.getDataType();
        const auto width = datatype.getSize();
        if (width == 0) {
            throw std::runtime_error("train_feature_ids has invalid fixed string width");
        }

        std::shared_ptr<char[]> raw_data(new char[obj->number_of_base_ * width]);
        feature_ids_dataset.read(raw_data.get(), datatype, feature_ids_dataspace);
        obj->train_feature_ids_ = std::shared_ptr<std::string[]>(new std::string[obj->number_of_base_]);
        for (int64_t i = 0; i < obj->number_of_base_; ++i) {
            const char* row = raw_data.get() + i * width;
            const void* zero = std::memchr(row, '\0', width);
            const auto length = zero == nullptr ? width : static_cast<size_t>(static_cast<const char*>(zero) - row);
            obj->train_feature_ids_[i].assign(row, length);
        }
    }
...
EvalDataset::Save(const EvalDatasetPtr& dataset, const std::string& filename) {
    ...
    if (dataset->train_feature_ids_) {
        size_t max_feature_id_len = 1;
        for (int64_t i = 0; i < dataset->number_of_base_; ++i) {
            max_feature_id_len = std::max(max_feature_id_len, dataset->train_feature_ids_[i].size());
        }

        std::vector<char> buffer(static_cast<size_t>(dataset->number_of_base_) * max_feature_id_len, 0);
        for (int64_t i = 0; i < dataset->number_of_base_; ++i) {
            const auto& feature_id = dataset->train_feature_ids_[i];
            std::memcpy(buffer.data() + i * max_feature_id_len,
                        feature_id.data(),
                        std::min(feature_id.size(), max_feature_id_len));
        }

        hsize_t dims[1] = {static_cast<hsize_t>(dataset->number_of_base_)};
        DataSpace dataspace(1, dims);
        StrType str_type(PredType::C_S1, max_feature_id_len);
        DataSet dataset_h5 = file.createDataSet("/train_feature_ids", str_type, dataspace);
        dataset_h5.write(buffer.data(), str_type);
    }
}
}  // namespace vsag::eval
