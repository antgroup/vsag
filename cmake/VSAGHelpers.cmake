...
# Include dependency declarations without inheriting VSAG's directory-level build options.
# The options are restored before VSAG-owned targets and subdirectories are declared.
macro (vsag_include_without_directory_options file)
    get_directory_property (_vsag_saved_compile_options COMPILE_OPTIONS)
    get_directory_property (_vsag_saved_link_options LINK_OPTIONS)
    set_property (DIRECTORY PROPERTY COMPILE_OPTIONS "")
    set_property (DIRECTORY PROPERTY LINK_OPTIONS "")
    include (${file})
    set_property (DIRECTORY PROPERTY COMPILE_OPTIONS ${_vsag_saved_compile_options})
    set_property (DIRECTORY PROPERTY LINK_OPTIONS ${_vsag_saved_link_options})
    unset (_vsag_saved_compile_options)
    unset (_vsag_saved_link_options)
endmacro ()
...
function (vsag_link_openmp_private target)
    if (NOT target)
        return ()
    endif ()
    if (TARGET "${target}" AND TARGET vsag_openmp)
        target_link_libraries ("${target}" PRIVATE vsag_openmp)
    endif ()
endfunction ()
