...
main(int argc, char** argv) {
    ...
    int64_t topk = 10;
    auto result = index->KnnSearch(query, topk, timeout_search_parameters).value();

    auto stats = result->GetStatistics({"is_timeout"});
    std::cout << "Search with timeout_ms=0: is_timeout = " << stats[0] << std::endl;
    ...
    result = index->KnnSearch(query, topk, normal_search_parameters).value();

    stats = result->GetStatistics({"is_timeout"});
    std::cout << "Search without timeout_ms: is_timeout = " << stats[0] << std::endl;
