...
main(int argc, char** argv) {
    ...
    std::vector<int16_t> datas(num_vectors * dim);
    std::mt19937 rng(47);
    std::uniform_int_distribution<int16_t> distrib_int16(std::numeric_limits<int16_t>::min(),
                                                         std::numeric_limits<int16_t>::max());
    for (int64_t i = 0; i < num_vectors; ++i) {
        ids[i] = i;
    }
    for (int64_t i = 0; i < dim * num_vectors; ++i) {
        datas[i] = distrib_int16(rng);
    }
    auto base = vsag::Dataset::Make();
    base->NumElements(num_vectors)
        ->Dim(dim)
        ->Ids(ids.data())
        ->BFloat16Vectors(datas.data())
        ->Owner(false);
    ...
    std::vector<int16_t> query_vector(dim);
    for (int64_t i = 0; i < dim; ++i) {
        query_vector[i] = distrib_int16(rng);
    }
    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(dim)->BFloat16Vectors(query_vector.data())->Owner(false);
