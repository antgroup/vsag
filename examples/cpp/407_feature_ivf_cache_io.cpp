...
main(int argc, char** argv) {
    ...
    std::string precise_file_path = argc > 1 ? argv[1] : "vsag_ivf_cache_io_precise";

    /******************* Create IVF Index with CacheIO on reorder codes *****************/
    ...
    if (auto build_result = index->Build(base); build_result.has_value()) {
        std::cout << "After Build(), Index IVF with reorder CacheIO contains: "
