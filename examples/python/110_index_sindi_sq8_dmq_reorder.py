...
This example demonstrates how to build and search a SINDI index with:
  ...
  - DMQ8 reorder enabled for high-precision distance computation (rerank_type: "dmq", dmq_bits: 8)
...
def sindi_sq8_dmq8_reorder_test():
    ...
    The configuration enables:
      ...
      - "rerank_type": "dmq"      -> Use DMQ backend for reranking
      - "dmq_bits": 8             -> 8-bit quantized codes per term (only valid value with dmq)
    """
    ...
    index_params = json.dumps(
        {
            ...
            "index_param": {
                ...
                "use_reorder": True,       # Enable reranking phase
                "rerank_type": "dmq",      # Use DMQ backend
                "dmq_bits": 8,             # 8-bit quantization (only valid value)
                "doc_prune_ratio": 0.0,
                "window_size": 100000,
            },
        }
    ...
    print("  - rerank_type: dmq         (DMQ backend)")
    print("  - dmq_bits: 8              (8-bit quantization)")
    print("\nBenefits:")
