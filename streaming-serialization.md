...
6. **能力选择加载**：写出可包含多种能力（如高/低精度 codes、raw vector、attribute filter），加载时按参数只取子集。

核心手段：头部 `StreamHeader` 承载全部语义元信息；body 采用
**TLV（Tag–Length–Value）自描述分段**。`Length` 让"无 seek 前向跳过"成为可能，
从而用同一套机制满足版本兼容与能力选择两类需求。
...
| G6 | 能力选择加载 | 各能力独立成块；加载参数生成 LoadPlan，跳过不需要的块 |
...
1. 根据索引状态和 `StreamSerializeOptions` 收集 header metadata、生成 block manifest。
...
## 7. 读端：LoadPlan + TLV 驱动循环

### 7.1 LoadPlan

读端解析 header 后、读 body 前，结合 `block_manifest` 与
`StreamDeserializeOptions` 生成 `LoadPlan`，对每个 tag 预先归类：

- 必须反序列化的块（critical 且本版本支持）。
- 按加载参数需要反序列化的块。
- 应当跳过的块（不需要的能力、未知的非 critical 块）。
- 缺失时可按旧语义补默认的块。
...
plan.CheckRequiredBlocks();                        // 所有 critical 块均已加载
...
- 如果用户加载参数显式要求某个低版本产物中不存在的能力
  （如 `load_raw_vector = true`）→ 报错，或按用户显式指定的 fallback 策略降级。

### 8.3 同一套"跳过"机制

前向兼容（被动跳过未知块）与能力选择（主动跳过不需要的块）复用**同一条**
TLV 前向跳过路径，差别只在 `ShouldDeserialize(tag)` 的判定来源：
...
写出与加载的能力集合可以不同。能力以独立 block 承载，加载参数决定取舍：
...
> 后续可演进为 capability bitset 或 JSON `load_params`
> （如 `{"load": {"precision": "low"}}` / `{"skip_tags": [...]}`），
> 避免 bool 字段无限增长。
...
- 加载 `high_precision_codes` 时，支撑搜索的 graph 与 label_table 必须已加载
  （由 critical 块保证）。
...
| 编排 | `src/algorithm/inner_index_interface.{h,cpp}` | 新增 `SerializeStreaming/DeserializeStreaming` 编排；**新增独立的 body 虚接口**，与旧路径隔离，回归风险最低 |
...
virtual tl::expected<void, Error>
SerializeStreaming(WriteFuncType write_func,
                   const StreamSerializeOptions& options = {}) const;
...
virtual void
SerializeStreamingBody(StreamWriter& writer,
                       const StreamSerializeOptions& options) const;
...
- **M2** 接口编排 + 公共 API（含 options/`load_params`）；以 BruteForce 或
  WARP 打通最小端到端闭环。
...
| 用户请求的能力在产物中缺失 | `INVALID_ARGUMENT` |
...
6. **后向兼容**：构造缺块/缺字段产物 → 按 `_version` 与参数快照补默认，
   行为与低版本一致。
7. **能力选择**：写高+低精度，分别只加载低 / 只加载高 → 搜索可用、内存更小；
   请求缺失能力 → `INVALID_ARGUMENT`。
8. **分块编码（T3）**：写读 + 跳过路径。
...
> `SECTION_END` 哨兵。`tag` 标识块、`value_len` 支撑"无 seek 前向跳过"、
> `flags.CRITICAL` 区分可忽略/必须报错、header 参数快照驱动后向默认——
> 由此用同一套机制同时拿下**前向兼容**、**后向兼容**与**能力选择加载**。
> 写端用 T1 测量预扫描（或 T3 分块编码）在无 seek 下取得长度，
> 读端用 LoadPlan + 有界前向读全程不 seek。
