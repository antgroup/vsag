...
*图示：现有格式：元信息在尾部，反序列化必须 Seek 到流尾才能读到 Footer*
...
*图示：新格式总体布局：Head 头部元信息 + TLV Body + SECTION_END，写读全程只向前*
...
*图示：两层可扩展性：分段级 TLV 增删跳过整块 + 字段级 JSON 增删标量参数*
...
*图示：TLV 记录字节布局：tag | block_version | flags | value_len | payload_checksum | payload*
...
*图示：T1 推荐方案：先整体测量预扫描得到每块尺寸，再一趟流式真正写出（零额外内存）*
...
- `deserialize`：无参数、全量恢复。输入是一个完整字节流，输出语义等价的内存态索引。
  ...
  当前版本完整理解，应明确报错。
- `Load`：有字符串参数、策略化加载。它是版本前后向兼容、能力选择、内存/磁盘态混合加载的
  唯一入口。`load` 可把某些块立即载入内存，把某些块绑定为 reader 延迟加载，也可按
  用户策略跳过可选能力。

### 7.1 LoadPlan

`Load` 解析 header 后、读 body 前，结合 `block_manifest` 与加载参数 JSON 字符串生成
`LoadPlan`，对每个 tag 预先归类。该字符串复用 `CreateIndex` 中已有的 IO 参数键，
例如 `base_io_type`、`base_file_path`、`precise_io_type`、`precise_file_path`、
`raw_vector_io_type`、`raw_vector_file_path`：
...
*图示：LoadPlan 决策流程图：按 block 是否被 reader 支持、是否 critical、是否被加载选项请求，决定反序列化、丢弃或报错*

### 7.2 读循环

*图示：反序列化读循环（全程前向）：解析 Head 后进入 TLV 循环，对每条记录派发/跳过/报错*

```cpp
auto header = StreamHeader::Read(reader);          // 顺序读，校验 magic/version/crc
auto plan = BuildLoadPlan(header, load_parameters);  // body 开始前确定
...
*图示：同一产物被两类读者处理：低版本读者前向兼容跳过未知块，选择性读者按加载参数空读跳过不需要的块*
...
真正需要策略的入口是 `Load`。能力以独立 block 承载，`Load` 的字符串参数决定每个能力的加载形态：

```json
{
  "base_io_type": "memory_io",
  "base_file_path": "./base.codes",
  "precise_io_type": "reader_io",
  "precise_file_path": "./precise.codes",
  "raw_vector_io_type": "mmap_io",
  "raw_vector_file_path": "./raw_vector.bin"
}
```

也可在字符串中追加更高层的 `load` 策略字段，例如
`{"load": {"precise_codes": "reader", "raw_vector": "skip"}}`。`serialize` / `deserialize`
不接受这些策略参数。
