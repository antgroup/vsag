...
- 正排 rerank 数据：如果 `use_reorder_` 为 true，写入 `rerank_backend_`。

backend 选择在 `SINDI` 构造函数完成：

- `rerank_type=fp32`: 使用 `SINDIFp32RerankBackend`，内部是一个排序后的 `SparseIndex`。
- `rerank_type=dmq`: 使用 `SINDIDmqRerankBackend`，存压缩后的 direct 8-bit DMQ 正排。

DMQ 有一个批量写入优化：`Add` 中会先收集本批成功插入的 `SparseVector` 和 label，循环结束后一次调用 `rerank_backend_->Add(dmq_base)`。FP32 backend 则逐条构造单元素 dataset 写入。
...
3. 如果 `use_reorder_` 为 true：
   - 调用 `rerank_backend_->PrepareQuery(...)` 生成 query context。
...
`remap_term_ids=true` 时，倒排 query 使用 remap 后的 compact term id；但 reorder backend 存的是原始向量，所以 `search_impl` 会通过 `original_query` 把原始 query 传给 rerank。

## FP32 Rerank Backend

`SINDIFp32RerankBackend` 是精确正排：

- `Add`: 委托内部 `SparseIndex::Add`。
- `PrepareQuery`: 对 query sparse vector 按 term id 排序。
- `CalDistanceByInnerId`: 调用 `SparseIndex::CalDistanceByIdUnsafe`。
- `Serialize` / `Deserialize`: 直接委托内部 `SparseIndex`。
- `GetSparseVectorByInnerId` / `CalcDistanceById` / `CalDistanceById`: 走内部 `SparseIndex`。

基础测试 `SINDI Basic Test` 会用 `SparseIndex` 作为 ground truth，验证开启 reorder 后的 SINDI 结果与精确 sparse index 一致。
...
- `SINDI Basic Test`: `use_reorder=true` + FP32 backend，对齐 `SparseIndex` ground truth，并覆盖序列化。
...
- `SINDI Remap with Reorder Test`: 覆盖 `remap_term_ids=true` 与 reorder 一起使用时，搜索结果与 `SparseIndex` 一致。
