...
void rotate_data_randomly(float *data, uint64_t num_points, uint64_t dim, float *rot_mat, float *&new_mat,
                          bool transpose_rot)
{
...
void compute_closest_centers_in_block(const float *const data, const uint64_t num_points, const uint64_t dim,
                                      const float *const centers, const uint64_t num_centers,
                                      const float *const docs_l2sq, const float *const centers_l2sq,
                                      uint32_t *center_index, float *const dist_matrix, uint64_t k)
{
    if (k > num_centers)
    {
...
void compute_closest_centers(float *data, uint64_t num_points, uint64_t dim, float *pivot_data, uint64_t num_centers,
                             uint64_t k, uint32_t *closest_centers_ivf, std::vector<uint64_t> *inverted_index,
...
void process_residuals(float *data_load, uint64_t num_points, uint64_t dim, float *cur_pivot_data, uint64_t num_centers,
...
#pragma omp parallel for schedule(static, 8192)
    for (int64_t n_iter = 0; n_iter < (int64_t)num_points; n_iter++)
    {
        for (uint64_t d_iter = 0; d_iter < dim; d_iter++)
        {
            if (to_subtract == 1)
                  ...
                  std::vector<uint64_t> *closest_docs, uint32_t *&closest_center)
{
    ...
    if (closest_docs == NULL)
        closest_docs = new std::vector<uint64_t>[num_centers];
    else
...
//#pragma omp parallel for schedule(static, 1)
    for (int64_t c = 0; c < (int64_t)num_centers; ++c)
    {
        ...
        for (uint64_t i = 0; i < dim; i++)
            cluster_sum[i] = 0.0;
        for (uint64_t i = 0; i < closest_docs[c].size(); i++)
        {
...
// vector<uint64_t> [num_centers], and closest_center = new uint64_t[num_points]
// Final centers are output in centers as row major num_centers * dim
//
float run_lloyds(float *data, uint64_t num_points, uint64_t dim, float *centers, const uint64_t num_centers,
                 const uint64_t max_reps, std::vector<uint64_t> *closest_docs, uint32_t *closest_center)
{
...
void selecting_pivots(float *data, uint64_t num_points, uint64_t dim, float *pivot_data, uint64_t num_centers)
{
    ...
    std::uniform_int_distribution<uint64_t> distribution(0, num_points - 1);
...
void kmeanspp_selecting_pivots(float *data, uint64_t num_points, uint64_t dim, float *pivot_data, uint64_t num_centers)
{
    if (num_points > 1 << 23)
    {
