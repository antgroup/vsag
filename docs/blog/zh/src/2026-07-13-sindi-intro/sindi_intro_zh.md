...
$$
\operatorname{score}(q,d)=\sum_{t \in q \cap d}q_t \cdot d_t
$$
...
第一部分是 **`倒排索引压缩`** 。前面提到过，SINDI 通过 window 组织倒排列表，可以把局部 doc id 从 `uint32_t` 压到 `uint16_t`。除此之外，SINDI 还可以用 **`SQ8（Scalar Quantization 8-bit，8 位标量量化）`** 把倒排索引里的 value 压缩到 8-bit。查询粗排时直接用量化后的 value 参与打分，不需要先解码成 float；并且，**倒排量化结合重排后对最终精度基本无损**。
...
|  | `use_quantization` | `index_param` | `false`、`true` 或 `"fp16"` / `"sq8"` / `"fp32"`，默认 `false` | 是否对倒排索引里的 value 开启量化以降低内存；`true` 等价于 `"sq8"`，`false` 等价于 `"fp32"`。 | 内存敏感场景建议使用 `true` 或 `"sq8"`；极致精度基线可使用 `false` 或 `"fp32"`。 |
...
| **可选（搜索）** | `n_candidate` | `sindi` | `[0, 500 * topk]`，默认 `0` | **`粗筛候选堆`** 大小；为 `0` 时等价于 `topk`，显式设置时不能超过 `500 * topk`。 | 开启重排时通常设为 `10 * topk` 到 `50 * topk` 起步；召回不足时调大，延迟过高时调小。 |
