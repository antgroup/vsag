...
"ivf": {
    ...
    "factor": 2.0,
    "parallelism": 4,
    "max_time_cost_ms": 30.0
}
```
means that the search will scan 10 buckets, the factor is 2.0, and the parallelism is 4, around 4 threads per query, and the max time cost is 30ms (when search time execeed 30ms, will return the current result). 
