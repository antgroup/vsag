...
excluding the build-generated `version.h` header in `src/` and vendored
`include/vsag/expected.hpp`. The version header is generated from `src/version.h.in`
and is absent in a clean checkout. Code compiled only
