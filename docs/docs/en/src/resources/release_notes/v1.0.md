...
- `KnnSearch` and `RangeSearch` handle direct KNN and range queries. When a request needs combined
  query modes, filters, or diagnostic inputs, `SearchRequest` and `Index::SearchWithRequest`
  collect JSON parameters, `FilterPtr`/bitset/attribute filters, and reasoning inputs in one
  object. Field and mode support varies by index.
