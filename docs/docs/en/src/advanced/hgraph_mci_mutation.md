...
`mci_incremental_clique_max` for both the threshold and a separate storage cap. Full builds
retain each enumerated maximal clique in full, bounded only by the candidate pool;
`mci_clique_max` is not a storage cap. Alpha expansion uses the same `next_mci_alpha` policy;
