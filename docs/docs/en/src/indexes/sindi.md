...
3. **Scoring.** At query time, SINDI iterates the non-zero terms of the query,
   ...
   forward store keeps fp32 values, while `rerank_type: "dmq"` uses a compressed
...
| `rerank_type` | string | `"fp32"` | Forward-store type used when `use_reorder` is enabled. `fp32` keeps exact values; `dmq` stores compressed 8-bit DMQ codes. |
| `dmq_bits` | int | `8` | DMQ code width. Currently only `8` is supported and is valid only with `rerank_type: "dmq"`. |
...
- Memory-constrained deployments of sparse corpora (`use_quantization: true`
  ...
  true` trades forward-store memory for recall, and `rerank_type: "dmq"` reduces
...
3. Pruned high-accuracy index with compressed reranking. Use the same pruning and
     inverted-list quantization as above, but set `rerank_type: "dmq"` and
     `dmq_bits: 8` together with `use_reorder: true` to reduce forward-store
     memory.
