...
- skip_strategy (default "deterministic_accumulative"): Determines how skipping is distributed:
  - "random": Random skipping. Each point independently has a 1 - skip_ratio probability of having its filter checked.
  - "deterministic_accumulative": Deterministic cumulative skipping. Skips at fixed intervals, guaranteeing that exactly skip_ratio * N points are skipped out of every N points.
...
- That is: all valid points are visited, plus a portion of invalid points

#### Usage Examples

```cpp
// Aggressive skipping: skip 10% of invalid point checks, suitable for latency-sensitive 
// scenarios where recall degradation is acceptable
...
// Conservative setting: only skip 50% of invalid points, less recall loss
