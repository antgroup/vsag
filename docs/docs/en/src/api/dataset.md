...
For single-query KNN, `GetNumElements()` is `1`; use `GetDim()` as the actual ids/distances array
length because fewer than the requested `k` neighbors may be returned. HGraph
