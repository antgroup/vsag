...
The factory in `src/datacell/flatten_datacell_factory_impl.h` dispatches to
...
- **HGraph** exposes the richest set: `base_quantization_type`,
  ...
  (see `src/algorithm/hgraph/hgraph_param_mapping.cpp`).
...
- **Pyramid** exposes `base_quantization_type`, `base_pq_dim`, the common
  reorder keys, the RabitQ PCA, base/query bit, and FHT keys, and all five
  `saq_*` keys.
