...
- 暴力搜索索引 **BruteForce**：支持单向量和多向量精确检索，
  是精确检索基线，也是小规模集合的精确检索选项。
...
- `KnnSearch`、`RangeSearch` 用于直接的 KNN/range 查询；需要组合查询模式、过滤和
  诊断输入时，可使用 `SearchRequest` 和 `Index::SearchWithRequest`，在一个请求对象中
  描述 JSON 参数、`FilterPtr`/bitset/属性过滤和 reasoning 输入。
  可用字段与模式取决于具体索引。
