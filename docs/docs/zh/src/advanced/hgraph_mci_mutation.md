...
不再以加入三个团为停止条件，而是持续加入直到当前点的去重邻居度数达到目标。

度数是覆盖该点的所有团的存活成员并集大小，排除自身，跨团重复邻居只计一次。
不能增加新邻居的团会被跳过。目标为：

```text
target = min(N - 1, max(degree_min, min(N / n_divisor, mcs / mcs_divisor)))
```

N 为当前存活点数，包含已完成图插入的整个 ADD 批次，不包含 MARK_REMOVE 的点。
整数除法向下取整；N≤1 时目标为 0。
`mci_incremental_degree_n_divisor` 默认 10000，
`mci_incremental_degree_mcs_divisor` 默认 2。
`degree_min` 对应参数 `mci_incremental_degree_min`，默认 50，必须为正整数。
因此默认 N=10000、mcs=200 时目标是 50，N=3241378 时目标是 100。
即使 mcs/2 小于配置的下限，仍以下限保底，但现有候选数量和覆盖限制仍然有效。
这是停止目标而非硬上限，加入完整团可能超过目标。
下限随索引参数序列化，加载时校验一致性；缺少该字段的旧配置采用默认值 50。
新建索引时显式设为 100 可请求此前的固定下限；加载已有索引仍须匹配其存储参数。
...
2. 构团门槛为 `max(2, min(clique_max, candidate_limit + 1, visible_total))`；
   增量路径使用 `mci_incremental_clique_max` 作为门槛和独立的保存大小上限，不再以凑够两个点结束扩张。
   全量构建完整保存枚举出的极大团，大小仅受候选池限制；`mci_clique_max` 不是保存大小上限。
...
JOIN 后若仍有度数缺口，从尚未属于当前点邻居的 KNN 中构团，随后重新统计度数。
重复直到目标达到、候选耗尽或度数不再增长，避免反复构建相同邻居集合。
覆盖次数限制和插入前缀可见范围可能使目标无法达到；没有可用邻居时仍以单点团保证覆盖。
删除修复点在原有“小团、低所属团数”筛选后，同样使用该度数策略。

该过程强调局部组织质量和覆盖，不承诺找到全局最大团或唯一极大团。
下文历史测量早于共用构团核心及度数目标策略的修改，不代表修改后的性能。
...
- `T_size = mci_delete_clique_size_threshold`，默认 `30`。
- `T_mct = mci_delete_node_mct_threshold`，默认 `3`。

默认大小阈值下，受影响团剩 29 个有效成员时废弃，剩 30 个时保留；不会扫描废弃无关的小团。
这是删除后的团废弃阈值，不是构建团大小上限；修复点的覆盖团数阈值仍为 3。
若将构建团大小上限设为小于 30，删除触及的这类团都会被废弃，应配套评估或调整本阈值。
显式配置的值（包括 3）保持有效，加载已有索引时仍需匹配其序列化参数。
下文历史实验保留当时配置，不代表新默认值 30 的性能结果。
...
MARK_REMOVE 将整批删除 ID 一次传给 PrepareDelete/CommitDelete，包括全量重建后
重新应用删除标记的场景。修复候选先去重，再按整批删除后的状态统一修复。
...
量化和其他 dtype 仍使用全扫描的精确候选；有界 top-K 堆只保留 mcs 个候选，
将候选缓存从 O(N) 降为 O(mcs)，不改变距离和 ID 排序结果。
单个修复点仍需 O(N) 次距离计算，R 个修复点仍需 O(RN) 次距离计算。
...
`PrepareDelete` 是作用于"受影响邻域"的两阈值单遍过滤：既不扫描全索引，也不迭代到不动点。

```text
退役(C)  ⟺  |live(C) \ D| < T_size
修复(v)  ⟺  |有效团(v) \ retiring| < T_mct
```

第二个条件是**投影值**：先把本批马上要废弃的团扣除，再与阈值比较。
若改用当前团数比较，就会漏掉"剩下的团恰好都是要废弃的团"的幸存点。
`retiring` 由整批一次扫描决定，因此改变 `D` 的遍历顺序不会改变结果；
`repair_node_ids` 按升序输出，保证修复顺序可复现。

以默认值（`T_size=30`、`T_mct=3`）删除节点 7 为例，7 属于 `C1`（删除前 31 个有效成员）、
`C2`（21）、`C3`（5）：

| 步骤 | 结果 |
| --- | --- |
| 受影响团 | `{C1, C2, C3}` |
| 删除后剩余有效成员 | `C1`：30，`C2`：20，`C3`：4 |
| 废弃（剩余 `< 30`） | `{C2, C3}`；`C1` 保留 |
| 修复候选 | `C2`、`C3` 中除 7 以外的有效成员，去重 |
| 投影团数 | 属于 `{C2, C3, C9}` 的候选为 1 → 修复；属于 `{C2, C4, C5, C6}` 的为 3 → 跳过 |

选出的集合只是**上界**：每次真正修复前，`repair_mci_clique_if_undercovered`
会重新读取当前覆盖团数，因此同一批中前面的修复可能已经满足覆盖、从而跳过后面的候选。
修复过程不会再废弃新的团。

候选集选取的代价只与受影响邻域有关，与索引规模无关：
...
真正昂贵的是选取之后：纯 FP32 修复对每个选中的点做一次近似 KNN 加一次局部建团；
非 FP32 修复对每个选中的点做一次 O(N) 距离扫描。两个阈值的作用就是把选中的集合压小。

| 删除路径 | 进入 `PrepareDelete` 的目标 | ID 空间 |
| --- | --- | --- |
| MARK_REMOVE | 每次 `Remove()` 调用解析出的一个整批；已删除 label 被跳过 | 当前槽位 |
| FORCE_REMOVE | 每次 `Remove()` 调用解析出的一个整批；包含已软删除目标，以便回收其槽位 | 压紧前槽位（`old_total`）；修复点 ID 必须经 `old_to_new` 翻译 |
| 全量重建后重放 | label 删除集合中的全部 ID（重建已清空 MCI 标记） | 当前槽位 |

调参：调大 `T_size` 更少废弃（修复更少，但 CSR 中需要过滤的残留更多）；
调小则废弃更多（结构更干净，但修复更贵）。`T_mct` 是触发阈值而非事后下限，调大会修复更多点。
`T_size` 应与 `mci_clique_max`、`mci_incremental_clique_max` 协调：
若团大小上限小于 `T_size`，该尺寸的受影响团会全部被废弃。
...
2. 把 MCI 标为暂时不可用。
3. 对整批去重后的待删除 inner ID 统一执行快照和提交，再修复去重后的低覆盖存活点。
...
两种删除模式都对**一次 `Remove()` 调用解析出的整个目标集合**做一次
`PrepareDelete`/`CommitDelete`，快照粒度相同。差别在于进入该集合的目标及其 ID 空间：
MARK_REMOVE 解析 label 时跳过已删除目标，FORCE_REMOVE 还包含已软删除目标，并使用压紧前的槽位空间。
因此改变批次会改变快照的目标集合，从而改变修复顺序和最终团结构。
...
FORCE_REMOVE 会自动再 Flush 一次修复结果。ADD 与 MARK_REMOVE 共用运行时计数，
每累计 100 个成功增删向量自动压缩：ADD 逐点检查，大批 ADD 内也会触发；
MARK_REMOVE 保持完整批次的投影与修复，在批末检查一次。失败新增、重复或不存在的删除不计数。
压缩成功、全量构建、加载及 FORCE_REMOVE 批末压缩后计数清零。自动维护分配失败时保留有效 delta，
不让已完成的 ADD/MARK_REMOVE 因压缩失败而报错，下次成功增删重试。
不再提供公共 Flush API；未达到阈值时序列化仍包含 delta。
后文历史实验早于此策略，不代表自动压缩版本的新性能结果；历史显式 Flush 命令不适用于当前 API。
...
| 历史增量加入策略 | ratio_threshold=0.6，最多加入3团（现已改为度数目标） |
...
历史最大加入团数扫描已不再支持；当前 ADD 使用度数目标。`delete_mct` 是修复触发条件，不是最低度数保证。
...
这次历史实验的删除团大小阈值为 3，ADD 的 join ratio=0.6、最多加入3团、clique max=50。
当前 ADD 已改为可配置度数目标。
