...
搜索因此可以"继续"而不是"重新开始"。调用方完成所有迭代后，需要将最后一次调用标记为
*last search*，让索引释放迭代过程中的临时资源，**并由调用方自行 `delete` 这个 `IteratorContext`**。
...
// 6. 最后一页：标记 is_last_search，让索引做收尾
...
每次调用都会推进 `iter_ctx`；多次结果的并集就是按距离顺序、不重叠的延续序列。

## `SearchRequest` API

如果使用更新的 `SearchWithRequest` 入口，迭代式搜索通过 `SearchRequest` 上的三个字段控制：

| 字段 | 含义 |
|------|------|
| `enable_iterator_search_` | 置为 `true` 开启迭代模式 |
| `p_iter_ctx_` | `IteratorContext*` 的二级指针。首次调用时由索引创建，后续调用复用 |
| `is_last_search_` | 最后一次调用置 `true`，让索引做收尾 |

语义与 `SearchParam` API 完全一致，上下文同样由调用方负责销毁。
...
- **所有权。** `IteratorContext` 由调用方持有，忘记 `delete` 会泄漏内部搜索状态（堆、已访问位图、
  allocator 临时分配）。
- **最后一次调用。** 一定要在最后一次调用时设置 `is_last_search = true`（或
  `SearchRequest::is_last_search_`），让索引刷新延后的清理逻辑。漏掉这一步可能会让相关资源
  在上下文销毁前一直被持有。
