...
编码，给出所有内建量化器中最高的压缩率。在 HGraph、IVF 和 Pyramid 上，
`x+y` split 模式把底库码拆成 `x` 个过滤 bit 和 `y` 个 supplement bit：
图遍历或 IVF 桶扫描只使用 filter code，重排 / full-distance 阶段只额外读取
supplement bits。
...
- HGraph、IVF 或 Pyramid 上 `rabitq_bits_per_dim_base = x` 且
...
| `rabitq_bits_per_dim_base` | int | `1` | standard RaBitQ 下表示底库码每维位数；HGraph/IVF/Pyramid `x+y` split 下，这个外部 key 表示 `x`，即过滤阶段使用的 filter bits。范围 `[1, 8]`。 |
| `rabitq_bits_per_dim_precise` | int | 未设置 | HGraph/IVF/Pyramid split 模式 key。和 `base_quantization_type: "rabitq"`、`precise_quantization_type: "rabitq"` 一起出现时表示 `y`，即重排 / full-distance 阶段读取的 supplement bits。要求 `x + y <= 8`。 |
| `rabitq_error_rate` | float | `1.9` | HGraph/IVF/Pyramid split 搜索的默认 lower-bound 误差倍率；必须为有限正数，也可以在 `hgraph` 或 `pyramid` 搜索参数中按次覆盖。 |
...
`rabitq_bits_per_dim_base`、`rabitq_bits_per_dim_precise`、`rabitq_version`、
`rabitq_error_rate`、
...
并提供 `rabitq_bits_per_dim_precise`。HGraph、IVF 和 Pyramid 会自动选择
split datacell。
下面例子中，过滤阶段使用 `x = 3` 个 filter bits，重排只读取 `y = 5` 个
supplement bits：
...
- **先旋转。** 对未归一化数据，设 `rabitq_use_fht: true`，或在 `tq` 链路
  中包含 `rom` / `fht`。
- **精度优先时用 split 模式。** HGraph/IVF/Pyramid `x+y` split 保留 `x` bit
  快速过滤路径，再添加 `y` 个 supplement bits 用于重排；相对纯 1 比特，使用
  更多总 bit 时召回明显更高。
