...
fused 训练只请求聚类中心，跳过未被使用的最终全量标签分配。
未启用 scalar-code 优化构建会话的 Build/Add 路径（包括 FP32 ODescent）对输入只预编码一次，
整批校验成功后复用这些编码写入索引。因此当前输入批次会临时占用
`N × (4 + filter_code_bytes + supplement_code_bytes)` 字节，其中包含聚类 ID。
缓存写入完成后释放，不保留 raw vectors，也不增加搜索常驻内存。
