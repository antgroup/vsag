...
# Support custom download URLs for third-party libraries via VSAG_ third-party library _URL environment variables
# Example: VSAG_BOOST_URL=http://mirror.example.com/boost_1_67_0.tar.gz make release
ifdef VSAG_BOOST_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_BOOST_URL="${VSAG_BOOST_URL}"
endif
ifdef VSAG_OPENBLAS_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_OPENBLAS_URL="${VSAG_OPENBLAS_URL}"
endif
ifdef VSAG_SPDLOG_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_SPDLOG_URL="${VSAG_SPDLOG_URL}"
endif
ifdef VSAG_JSON_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_JSON_URL="${VSAG_JSON_URL}"
endif
ifdef VSAG_CATCH2_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_CATCH2_URL="${VSAG_CATCH2_URL}"
endif
ifdef VSAG_FMT_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_FMT_URL="${VSAG_FMT_URL}"
endif
ifdef VSAG_ANTLR4_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_ANTLR4_URL="${VSAG_ANTLR4_URL}"
endif
ifdef VSAG_CPUINFO_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_CPUINFO_URL="${VSAG_CPUINFO_URL}"
endif
ifdef VSAG_TSL_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_TSL_URL="${VSAG_TSL_URL}"
endif
ifdef VSAG_THREAD_POOL_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_THREAD_POOL_URL="${VSAG_THREAD_POOL_URL}"
endif
ifdef VSAG_ROARINGBITMAP_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_ROARINGBITMAP_URL="${VSAG_ROARINGBITMAP_URL}"
endif
ifdef VSAG_PYBIND11_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_PYBIND11_URL="${VSAG_PYBIND11_URL}"
endif
ifdef VSAG_HDF5_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_HDF5_URL="${VSAG_HDF5_URL}"
endif
ifdef VSAG_ARGPARSE_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_ARGPARSE_URL="${VSAG_ARGPARSE_URL}"
endif
ifdef VSAG_YAML_CPP_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_YAML_CPP_URL="${VSAG_YAML_CPP_URL}"
endif
ifdef VSAG_TABULATE_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_TABULATE_URL="${VSAG_TABULATE_URL}"
endif
ifdef VSAG_CPR_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_CPR_URL="${VSAG_CPR_URL}"
endif
ifdef VSAG_DISKANN_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_DISKANN_URL="${VSAG_DISKANN_URL}"
endif
ifdef VSAG_MKL_URL
  VSAG_CMAKE_ARGS := ${VSAG_CMAKE_ARGS} -DVSAG_MKL_URL="${VSAG_MKL_URL}"
endif
