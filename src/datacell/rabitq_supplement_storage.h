...
/**
 * Supplement (xbit) storage component for split RaBitQ.
 *
 * Holds the high-bit planes plus the trailing meta region of the encoded
 * vector. Because the supplement bytes are only needed during reorder /
 * full-distance computation, this storage is designed to cooperate with
 * disk-backed IO: callers can keep one-bit traversal hot while reorder
 * paths fetch supplement bytes on demand (single Read or batched MultiRead).
 */
template <typename IOTmpl>
class RaBitQSupplementStorage {
public:
    static constexpr bool InMemory = IOTmpl::InMemory;

    RaBitQSupplementStorage(const IOParamPtr& io_param, const IndexCommonParam& common_param)
