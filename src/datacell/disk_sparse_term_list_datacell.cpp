...
DiskSparseTermListDataCell<IOTmpl>::BuildTermDict(std::vector<DiskTermEntry>& term_dict,
    ...
    for (uint32_t term_id = 0; term_id <= term_id_limit_; ++term_id) {
        ...
        const auto posting_payload_size = current_offset - entry.posting_payload_offset;
        if (posting_payload_size > std::numeric_limits<uint32_t>::max()) {
            throw VsagException(
                ErrorType::INVALID_ARGUMENT,
                fmt::format("DiskSINDI posting payload for term {} exceeds uint32_t: {}",
                            term_id,
                            posting_payload_size));
        }
        entry.posting_payload_size = static_cast<uint32_t>(posting_payload_size);
