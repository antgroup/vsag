...
TEST_CASE("SparseTermDataCell Position Storage Caps At Uint16 Range",
    ...
    sv.ids_ = ids;
    sv.vals_ = vals;
    sv.token_seq_len_ = static_cast<uint32_t>(token_seq.size());
    sv.token_sequence_ = token_seq.data();

    cell.InsertVector(sv, 0);

    auto pos = cell.GetPositions(10, 0);
    REQUIRE(pos.size() == 65536);
    REQUIRE(pos.front() == 0);
    REQUIRE(pos.back() == std::numeric_limits<uint16_t>::max());
}
...
TEST_CASE("SparseTermDataCell Position Storage Memory Usage",
          "[ut][SparseTermDatacell][Position]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    SparseTermDataCell without_positions(1.0f,
                                         100,
                                         allocator.get(),
                                         SparseValueQuantizationType::FP32,
                                         nullptr,
                                         false,
                                         64);
    SparseTermDataCell with_positions(1.0f,
                                      100,
                                      allocator.get(),
                                      SparseValueQuantizationType::FP32,
                                      nullptr,
    ...
    vector.ids_ = ids;
    vector.vals_ = vals;
    vector.token_seq_len_ = 3;
    vector.token_sequence_ = token_seq;

    without_positions.InsertVector(vector, 0);
    with_positions.InsertVector(vector, 0);

    REQUIRE(with_positions.GetMemoryUsage() > without_positions.GetMemoryUsage());
}
