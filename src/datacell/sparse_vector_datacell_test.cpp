...
TEST_CASE("SparseDataCell FP16 values", "[ut][SparseDataCell][fp16]") {
    ...
    uint32_t boundary_ids[] = {1, 2, 3};
    float boundary_values[] = {0.0F, 0x1p-24F, 0x1p-25F};
    SparseVector boundary_vector{3, boundary_ids, boundary_values};
    fp16->InsertVector(&boundary_vector, 2);
    SparseVector decoded_boundary{};
    fp16->GetSparseVectorByInnerId(2, &decoded_boundary, allocator.get());
    REQUIRE(decoded_boundary.vals_[0] == 0.0F);
    REQUIRE(decoded_boundary.vals_[1] == 0x1p-24F);
    REQUIRE(decoded_boundary.vals_[2] == 0.0F);
    allocator->Deallocate(decoded_boundary.ids_);
    allocator->Deallocate(decoded_boundary.vals_);
