...
TEST_CASE("SparseDataCell nnz is independent of common dim", "[ut][SparseDataCell]") {
    auto param = std::make_shared<SparseVectorDataCellParameter>();
    ...
    std::vector<uint32_t> ids(17);
    std::iota(ids.begin(), ids.end(), 0);
    std::vector<float> values(17, 1.0F);
    SparseVector vector{static_cast<uint32_t>(ids.size()), ids.data(), values.data()};

    REQUIRE_NOTHROW(data_cell->InsertVector(&vector, 0));
    SparseVector decoded;
    data_cell->GetSparseVectorByInnerId(0, &decoded, index_common_param.allocator_.get());
    REQUIRE(decoded.len_ == vector.len_);
    index_common_param.allocator_->Deallocate(decoded.ids_);
    index_common_param.allocator_->Deallocate(decoded.vals_);
}
...
TEST_CASE("SparseDataCell Concurrent Test", "[ut][SparseDataCell][concurrent] ") {
    ...
    for (uint64_t i = base_count / 2; i < base_count; ++i) {
        futures.push_back(thread_pool.enqueue(
    ...
    for (uint64_t i = 0; i < base_count - 1; ++i) {
        fixtures::dist_t distance = data_cell->ComputePairVectors(idx[i], idx[i + 1]);
        REQUIRE(distance == fixtures::GetSparseDistance(sparse_vectors[i], sparse_vectors[i + 1]));
    }
    auto query_sparse_vectors = fixtures::GenerateSparseVectors(1, 100);
...
//   [uint64 offset_io_size][N × uint32 offset]
//   [quantizer payload]
...
write_legacy_format(std::stringstream& legacy_stream,
    ...
    uint64_t legacy_offset_io_size = legacy_entries.size() * sizeof(uint32_t);
    write(&legacy_offset_io_size, sizeof(legacy_offset_io_size));
    for (const auto& entry : legacy_entries) {
        write(&entry.first, sizeof(entry.first));
    }
