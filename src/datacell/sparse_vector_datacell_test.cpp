...
TEST_CASE("SparseDataCell Rejects Misaligned Legacy Offset Table", "[ut][SparseDataCell]") {
    constexpr const char* param_str = R"({
        "io_params": {"type": "memory_io"},
        "quantization_params": {"type": "sparse"}
    })";
    ...
    auto write = [&](const auto& value) {
        legacy_ss.write(reinterpret_cast<const char*>(&value), sizeof(value));
    };
    write(uint32_t{0});  // total_count
    write(uint32_t{0});  // max_capacity
    write(uint32_t{0});  // code_size
    write(uint32_t{0});  // legacy current_offset
    write(uint64_t{0});  // io size
    write(uint64_t{1});  // invalid legacy offset_io size

    legacy_ss.seekg(0, std::ios::beg);
    auto loaded = FlattenInterface::MakeInstance(param, index_common_param);
    IOStreamReader reader(legacy_ss);
    REQUIRE_THROWS_WITH(
        loaded->Deserialize(reader),
        Catch::Matchers::ContainsSubstring("invalid legacy SparseVectorDataCell offset size"));
}
