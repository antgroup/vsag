...
SparseVectorDataCell<QuantTmpl, IOTmpl>::query(float* result_dists,
    ...
    for (int i = 0; i < id_count; ++i) {
        ...
        auto codes = this->GetCodesByIdUnsafe(idx[i], need_release);
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::Deserialize(lvalue_or_rvalue<StreamReader> reader) {
    ...
    if (maybe_sentinel == SERIALIZE_FORMAT_SENTINEL) {
        ...
        max_code_size_ = sizeof(uint32_t);
        const auto doc_count = this->offset_io_->size_ / sizeof(DocLocation);
        for (uint64_t i = 0; i < doc_count; ++i) {
            DocLocation location;
            offset_io_->Read(
                sizeof(location), i * sizeof(location), reinterpret_cast<uint8_t*>(&location));
            max_code_size_ = std::max(max_code_size_, static_cast<uint64_t>(location.size));
        }
    } else {
        ...
        if (doc_count > 0) {
            ...
            while (remaining > 0) {
                ...
                for (uint64_t i = 0; i < batch; ++i) {
                    ...
                    max_code_size_ =
                        std::max(max_code_size_, static_cast<uint64_t>(new_batch[i].size));
                }
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::InsertVector(const void* vector, InnerIdType idx) {
    ...
    std::unique_ptr<uint8_t, decltype(deleter)> codes(
        reinterpret_cast<uint8_t*>(allocator_->Allocate(code_size)), deleter);
    quantizer_->EncodeOne((const float*)vector, codes.get());
    DocLocation location;
    {
        ...
        io_->Write(codes.get(), code_size, location.offset);
    }
}
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::GetCodesById(InnerIdType id, bool& need_release) const {
    std::shared_lock lock(mutex_);
    return this->GetCodesByIdUnsafe(id, need_release);
}

template <typename QuantTmpl, typename IOTmpl>
const uint8_t*
SparseVectorDataCell<QuantTmpl, IOTmpl>::GetCodesByIdUnsafe(InnerIdType id,
...
// fewer merges, and duplicate / overlapping candidates may re-read bytes.
template <typename QuantTmpl, typename IOTmpl>
BatchCodesResult
SparseVectorDataCell<QuantTmpl, IOTmpl>::GetCodesByIdsBatch(const InnerIdType* ids,
    ...
    for (uint64_t r = 0; r < num_ranges; ++r) {
        ...
        for (uint32_t i = rg.first; i <= rg.last; ++i) {
            ...
            std::memcpy(
                result.buffer.data() + result.in_buffer_offsets[i], base + local, result.sizes[i]);
        }
    }
    return result;
}
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::GetSparseVectorByInnerId(
    ...
    const auto* codes = this->GetCodesByIdUnsafe(inner_id, need_release);
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::ComputePairVectors(InnerIdType id1, InnerIdType id2) {
    ...
    auto* codes1 = this->GetCodesByIdUnsafe(id1, release1);
    auto* codes2 = this->GetCodesByIdUnsafe(id2, release2);
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::SparseVectorDataCell(
    ...
    const auto dim = common_param.dim_ > 0 ? static_cast<uint64_t>(common_param.dim_) : 0;
    this->max_code_size_ =
        std::max(static_cast<uint64_t>(sizeof(uint32_t)), (dim * 2 + 1) * sizeof(uint32_t));
