...
SparseVectorDataCell<QuantTmpl, IOTmpl>::Deserialize(lvalue_or_rvalue<StreamReader> reader) {
    ...
    if (maybe_sentinel == SERIALIZE_FORMAT_SENTINEL) {
        ...
        this->io_->Deserialize(reader);
        // Legacy offset_io_ holds one uint32 offset per vector. Expand each offset
        // to a DocLocation by reading the encoded sparse-vector length from io_.
        uint64_t legacy_offset_io_size = 0;
        StreamReader::ReadObj(reader, legacy_offset_io_size);
        const uint64_t legacy_entry_size = sizeof(uint32_t);
        ...
        if (doc_count > 0) {
            ...
            Vector<uint32_t> legacy_batch(allocator_);
            ...
            while (remaining > 0) {
                ...
                for (uint64_t i = 0; i < batch; ++i) {
                    const uint64_t offset = legacy_batch[i];
                    uint32_t len = 0;
                    CHECK_ARGUMENT(this->io_->Read(sizeof(len),
                                                   offset,
                                                   reinterpret_cast<uint8_t*>(&len)),
                                   "failed to read legacy sparse vector length");
                    const uint64_t code_size =
                        (static_cast<uint64_t>(len) * 2 + 1) * sizeof(uint32_t);
                    CHECK_ARGUMENT(code_size <= std::numeric_limits<uint32_t>::max(),
                                   "legacy sparse vector code size is too large");
                    CHECK_ARGUMENT(offset <= current_offset_,
                                   "legacy sparse vector offset is out of bounds");
                    CHECK_ARGUMENT(code_size <= current_offset_ - offset,
                                   "legacy sparse vector code range is out of bounds");
                    new_batch[i].offset = offset;
                    new_batch[i].size = static_cast<uint32_t>(code_size);
                }
...
SparseVectorDataCell<QuantTmpl, IOTmpl>::InsertVector(const void* vector, InnerIdType idx) {
    ...
    CHECK_ARGUMENT(code_size <= std::numeric_limits<uint32_t>::max(),
                   fmt::format("code size ({}) of sparse vector is too large", code_size));
