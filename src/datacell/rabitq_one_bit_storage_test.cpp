...
constexpr uint64_t kCodeSize = 16;
...
std::shared_ptr<RaBitQOneBitStorage<MemoryIO>>
MakeStorage(const IndexCommonParam& common_param, uint64_t code_size = kCodeSize) {
    ...
    auto storage = std::make_shared<RaBitQOneBitStorage<MemoryIO>>(io_param, common_param);
...
TEST_CASE("RaBitQOneBitStorage CodeSize", "[ut][RaBitQOneBitStorage]") {
    ...
    REQUIRE(RaBitQOneBitStorage<MemoryIO>::InMemory == MemoryIO::InMemory);

    storage->SetCodeSize(32);
    REQUIRE(storage->GetCodeSize() == 32);
}

TEST_CASE("RaBitQOneBitStorage Resize And Shrink", "[ut][RaBitQOneBitStorage]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    auto common_param = MakeCommonParam(allocator);
    auto storage = MakeStorage(common_param);
...
TEST_CASE("RaBitQOneBitStorage Write And Read", "[ut][RaBitQOneBitStorage]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    auto common_param = MakeCommonParam(allocator);
    auto storage = MakeStorage(common_param);
    ...
    for (uint64_t id = 0; id < kInitialCapacity; ++id) {
        for (uint64_t b = 0; b < kCodeSize; ++b) {
            records[id][b] = static_cast<uint8_t>((id * 31U + b) & 0xFFU);
        }
...
TEST_CASE("RaBitQOneBitStorage Prefetch And MultiRead", "[ut][RaBitQOneBitStorage]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    auto common_param = MakeCommonParam(allocator);
    auto storage = MakeStorage(common_param);
    ...
    std::iota(record.begin(), record.end(), static_cast<uint8_t>(1));
    for (uint64_t id = 0; id < kInitialCapacity; ++id) {
        storage->Write(record.data(), static_cast<InnerIdType>(id));
    }

    // Prefetch should not throw and tolerate over-large byte requests.
    storage->Prefetch(0, kCodeSize);
    storage->Prefetch(static_cast<InnerIdType>(kInitialCapacity - 1), kCodeSize * 4);

    constexpr uint64_t kBatch = 3;
...
TEST_CASE("RaBitQOneBitStorage Serialize And Deserialize", "[ut][RaBitQOneBitStorage]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    auto common_param = MakeCommonParam(allocator);
    auto storage = MakeStorage(common_param);
    ...
    std::vector<uint8_t> record(kCodeSize, 0);
    for (uint64_t b = 0; b < kCodeSize; ++b) {
        record[b] = static_cast<uint8_t>(b * 7U + 1U);
    }
    for (uint64_t id = 0; id < kInitialCapacity; ++id) {
        storage->Write(record.data(), static_cast<InnerIdType>(id));
    }
...
TEST_CASE("RaBitQOneBitStorage InitIO", "[ut][RaBitQOneBitStorage]") {
    auto allocator = SafeAllocator::FactoryDefaultAllocator();
    auto common_param = MakeCommonParam(allocator);
    auto storage = MakeStorage(common_param);
