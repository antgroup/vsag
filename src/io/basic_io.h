...
public:
   ...
   * @param size The size of the data to be read.
   * @param offset The offset at which to read the data.
   * @param need_release A reference to a boolean indicating whether the
   * returned data needs to be released.
   ...
   * Otherwise, a runtime error is thrown.
   *
   * @param datas An array of pointers to the buffers where the read data will
   * be stored.
   ...
   *
   * @param reader The StreamReader from which the IO object will be
   * deserialized.
   */
    inline void
   ...
   *
   * Sometimes, new buffer is malloced by read, so need release by use this
   * method.
   *
...
protected:
   ...
   *
   * This function checks if the given offset is within the bounds of the IO
   * object. If the offset is valid, the function returns true. Otherwise, it
   * returns false.
   *
...
private:
   ...
   */
    ...
    /**
   * @brief Casts the current object to the underlying IO object type (const
   * version).
   *
...
private:
    /**
   * @brief Generates a struct to check if a class has a member function with a
   * specific signature.
   *
   */
