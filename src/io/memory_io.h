...
public:
    ...
    explicit MemoryIO(const MemoryIOParameterPtr& param, const IndexCommonParam& common_param)
        : MemoryIO(common_param.allocator_.get()) {
    }

    explicit MemoryIO(const IOParameterPtr& param, const IndexCommonParam& common_param)
