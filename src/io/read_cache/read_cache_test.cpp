...
ReadCacheParameter
MakeCacheParam(uint64_t page_count = 4) {
    ReadCacheParameter param;
    param.total_cache_size_ = Page::DEFAULT_PAGE_SIZE * page_count;
    return param;
}
...
TEST_CASE("BasicIO cache component basic test", "[ReadCache][ut]") {
    ...
    io.EnableReadCache(std::make_shared<ReadCacheParameter>(MakeCacheParam()));
...
TEST_CASE("ReadCache parameter test", "[ReadCache][ut]") {
    JsonType json;
    json["type"].SetString("read_cache");
    json["total_cache_size"].SetUint64(4096);
    json["eviction_strategy"].SetString("lru");

    ReadCacheParameter param(json);
    REQUIRE(param.total_cache_size_ == 4096);
