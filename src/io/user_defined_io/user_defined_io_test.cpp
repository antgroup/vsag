...
vsag::UserDefinedIOPair
make_storage(const std::shared_ptr<ByteStore>& store) {
    return vsag::Factory::CreateUserDefinedIO(
...
std::unique_ptr<vsag::UserDefinedIO>
make_io(const std::shared_ptr<ByteStore>& store, bool cache = false) {
    auto storage = make_storage(store);
    auto storages = std::make_shared<vsag::UserDefinedIOSet>();
    storages->Set("test", storage.reader, storage.writer);
    auto param = std::make_shared<vsag::UserDefinedIOParameter>();
    param->user_defined_io = "test";
    ...
    common.user_defined_ios_ = storages;
    return std::make_unique<vsag::UserDefinedIO>(param, common);
}

}  // namespace

TEST_CASE("UserDefinedIO grow overwrite shrink and failures", "[ut][UserDefinedIO]") {
...
TEST_CASE("UserDefinedIO parameter reuse does not leak bound handles", "[ut][UserDefinedIO]") {
    ...
    auto parameter = std::make_shared<vsag::UserDefinedIOParameter>();
    parameter->user_defined_io = "shared_name";

    vsag::IndexCommonParam first_common;
    first_common.allocator_ = first->allocator;
    auto first_set = std::make_shared<vsag::UserDefinedIOSet>();
    first_set->Set("shared_name", first_storage.reader, first_storage.writer);
    first_common.user_defined_ios_ = first_set;

    vsag::IndexCommonParam second_common;
    second_common.allocator_ = second->allocator;
    auto second_set = std::make_shared<vsag::UserDefinedIOSet>();
    second_set->Set("shared_name", second_storage.reader, second_storage.writer);
    second_common.user_defined_ios_ = second_set;

    vsag::UserDefinedIO first_io(parameter, first_common);
    vsag::UserDefinedIO second_io(parameter, second_common);
...
TEST_CASE("UserDefinedIOSet rejects invalid registrations", "[ut][UserDefinedIO]") {
    auto store = std::make_shared<ByteStore>();
    auto storage = make_storage(store);
...
TEST_CASE("UserDefinedBackend preserves read error channels and skips empty reads",
          "[ut][UserDefinedIO]") {
    auto allocator = vsag::Engine::CreateDefaultAllocator();
    uint64_t reads = 0;
    auto storage = vsag::Factory::CreateUserDefinedIO(
    ...
    vsag::UserDefinedBackend backend(allocator.get());
    REQUIRE(backend.Initialize(param, false, 0, 0) == 1);
    auto strict_param = std::make_shared<vsag::UserDefinedIOParameter>(*param);
    strict_param->reader = vsag::Factory::CreateReadFuncReader(
        [](uint64_t, uint64_t, void*) { throw std::runtime_error("unexpected read"); }, 1);
    vsag::UserDefinedBackend strict_backend(allocator.get());
...
TEST_CASE("UserDefinedIO factory parsing preserves common configuration", "[ut][UserDefinedIO]") {
    auto original = std::make_shared<vsag::UserDefinedIOParameter>();
    original->user_defined_io = "test";
...
TEST_CASE("CallbackStorage rejects empty callbacks as invalid arguments", "[ut][UserDefinedIO]") {
    const auto read = [](uint64_t, uint64_t, void*) {};
    const auto write = [](uint64_t, uint64_t, const void*) {};
    const auto resize = [](uint64_t) {};
    REQUIRE_THROWS_AS(vsag::Factory::CreateUserDefinedIO({}, write, resize, 0),
                      std::invalid_argument);
    REQUIRE_THROWS_AS(vsag::Factory::CreateUserDefinedIO(read, {}, resize, 0),
                      std::invalid_argument);
    REQUIRE_THROWS_AS(vsag::Factory::CreateUserDefinedIO(read, write, {}, 0),
                      std::invalid_argument);
    auto store = std::make_shared<ByteStore>();
    auto storage = make_storage(store);
    storage.writer->Resize(1);
...
TEST_CASE("UserDefinedBackend submission completes synchronously", "[ut][UserDefinedIO]") {
    REQUIRE_FALSE(vsag::UserDefinedBackendCapabilities::AsyncReadable);
    auto store = std::make_shared<ByteStore>();
    auto storage = make_storage(store);
    storage.writer->Resize(1);
    auto param = std::make_shared<vsag::UserDefinedIOParameter>();
    param->reader = storage.reader;
    param->writer = storage.writer;
    vsag::UserDefinedBackend backend(store->allocator.get());
    REQUIRE(backend.Initialize(param, false, 0, 0) == 1);
    uint8_t byte = 1;
    const vsag::ReadRequest request{&byte, 0, 1};
...
TEST_CASE("UserDefinedIO rejects missing pairs", "[ut][UserDefinedIO]") {
    auto param = std::make_shared<vsag::UserDefinedIOParameter>();
    param->user_defined_io = "missing";
    vsag::IndexCommonParam common;
    common.allocator_ = vsag::Engine::CreateDefaultAllocator();
    common.user_defined_ios_ = std::make_shared<vsag::UserDefinedIOSet>();
    REQUIRE_THROWS(vsag::UserDefinedIO(param, common));
}
