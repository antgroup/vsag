...
LabelTable::LabelTable(Allocator* allocator, bool use_reverse_map, bool compress_redundant_data)
    ...
    deleted_ids_filter_ = std::make_shared<RemoveListFilter>(deleted_ids_);
}
...
LabelTable::GetIdByLabel(LabelType label, bool return_even_removed) const {
    ...
    if (not return_even_removed and this->deleted_ids_.count(id) > 0) {
        throw VsagException(ErrorType::INTERNAL_ERROR, fmt::format("label {} is removed", label));
    }
    return id;
}
...
LabelTable::MarkRemove(const std::vector<LabelType>& labels) {
    uint32_t init_delete_size = this->deleted_ids_.size();
