...
struct ReasoningCapabilityEntry {
    std::string_view factory_name;
    ReasoningCapability capability;
};

constexpr ReasoningCapabilityEntry K_REASONING_CAPABILITIES[] = {{"hgraph", K_HGRAPH_CAP},
                                                                 {"ivf", K_IVF_CAP},
                                                                 {"sindi", K_SINDI_CAP},
                                                                 {"sindi_v2", K_SINDI_V2_CAP},
                                                                 {"brute_force", K_BRUTE_FORCE_CAP},
                                                                 {"warp", K_WARP_CAP},
                                                                 {"pyramid", K_PYRAMID_CAP}};

}  // namespace

const ReasoningCapability*
GetReasoningCapability(std::string_view index_type) {
    for (const auto& entry : K_REASONING_CAPABILITIES) {
        if (index_type == entry.capability.index_type || index_type == entry.factory_name) {
