...
TEST_CASE("Reasoning capability registry is the support matrix", "[reasoning]") {
    SECTION("Known indexes resolve") {
        ...
        REQUIRE(bruteforce->supports_knn);
        ...
        REQUIRE(pyramid->supports_knn);
        REQUIRE_FALSE(pyramid->supports_range);
    ...
    SECTION("Factory aliases resolve to the canonical capability") {
        ...
        for (const auto& pair : names) {
            CAPTURE(pair[0]);
            const auto* capability = GetReasoningCapability(pair[0]);
            REQUIRE(capability != nullptr);
            REQUIRE(capability == GetReasoningCapability(pair[1]));
            REQUIRE(capability->index_type == pair[1]);
        }
    }
