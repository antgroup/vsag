...
enum class ReasoningDiagnosis : uint8_t {
    ...
    kReorderEvicted,
    kUnknown,
    // Keep values contiguous; insert new diagnoses before this non-reportable sentinel.
    kCount,
};
...
enum class ReasoningEvent : uint8_t {
    ...
    kReorderEviction,
    kBucketSelection,
    // Keep values contiguous; insert new events before this non-reportable sentinel.
    kCount,
};
static_assert(static_cast<uint8_t>(ReasoningEvent::kCount) <= 64,
              "Reasoning events must fit in the capability mask");

enum class ReasoningReportStatus : uint8_t {
    kOk = 0,
    kSkippedRangeSearch,  // Reserved for future status-only range responses.
    kUnsupportedByIndex,  // Reserved for future unsupported-index responses.
    kEmptyIndex,
};
