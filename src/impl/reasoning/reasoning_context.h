...
//   * What does an index record? GetReasoningCapability(index_type), defined in
//     reasoning_capability.cpp.
//   * Where are the hooks?       Every instrumentation site is tagged with a
//     "// [reasoning]" comment; `grep -rn "\\[reasoning\\]" src/` lists them.
//
// How to add a new diagnosis:
//   1. add a value to ReasoningDiagnosis (reasoning_types.h)
//   2. map it in ToString (reasoning_types.cpp)
//   3. add a rule in DiagnoseTarget (reasoning_context.cpp)
//   4. add a SECTION in reasoning_context_test.cpp
//   5. add a row in docs/docs/{en,zh}/src/advanced/search-reasoning.md
//
// How to add a new event type:
//   1. add a value to ReasoningEvent (reasoning_types.h)
...
public:
    ...
    static std::string
    MakeStatusReport(ReasoningReportStatus status, std::string_view index_type);
