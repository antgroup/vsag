...
BasicSearcher::search_impl(const GraphInterfacePtr& graph,
    ...
    auto push_result = [&](InnerIdType id, float distance) {
        if (not iter_ctx->CheckPoint(id) or not is_result_allowed(id) or
            not is_result_distance_eligible<mode>(distance, inner_search_param)) {
            return false;
        }
        ...
        while (!iter_ctx->Empty()) {
            ...
            if (iter_ctx->CheckPoint(cur_inner_id)) {
                ...
                if (not vl->TestAndSet(traversal_id)) {
                    candidate_set->Push(traversal_priority(cur_dist), traversal_id);
                }
            }
            iter_ctx->PopDiscard();
        }
