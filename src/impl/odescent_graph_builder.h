...
struct Linklist {
    ...
    Linklist(Allocator* allocator)
        : neigbors(allocator), greast_neighbor_distance(std::numeric_limits<float>::max()) {
    }
};
...
private:
    ...
    void
    update_neighbors(Vector<Vector<uint32_t>>& old_neigbors,
    ...
    void
    sample_candidates(Vector<Vector<uint32_t>>& old_neigbors,
                      Vector<Vector<uint32_t>>& new_neigbors,
                      float sample_rate);
