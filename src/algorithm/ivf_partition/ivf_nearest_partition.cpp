...
IVFNearestPartition::factory_router_index() {
    auto allocator = std::make_shared<SafeAllocator>(allocator_);
    IndexCommonParam param;
    param.dim_ = this->dim_;
    param.metric_ = MetricType::METRIC_TYPE_L2SQR;
    param.data_type_ = DataTypes::DATA_TYPE_FLOAT;
    ...
    this->route_index_ptr_ = std::make_shared<BruteForce>(param_ptr, param);
}
}  // namespace vsag
