...
Pyramid::Serialize(StreamWriter& writer) const {
    ...
    if (this->persist_source_id_) {
        this->serialize_source_id_table(writer);
    }
...
Pyramid::collect_streaming_header() const {
    ...
    if (this->persist_source_id_) {
        auto source_id_tag = static_cast<uint32_t>(StreamSerializationTag::SOURCE_ID_TABLE);
        AppendStreamingManifestBlock(manifest,
                                     source_id_tag,
                                     StreamSerializationBlockCurrentVersion(source_id_tag),
                                     StreamSerializationTagCritical(source_id_tag));
    }
    AppendStreamingManifestBlock(manifest,
                                 base_tag,
...
void
Pyramid::deserialize_source_id_table(StreamReader& reader) const {
...
Pyramid::serialize_streaming_body(StreamWriter& writer) const {
    ...
    if (this->persist_source_id_) {
        ...
        WriteStreamingBlock(writer,
                            source_id_tag,
                            StreamSerializationTagCritical(source_id_tag),
                            [this](StreamWriter& w) { this->serialize_source_id_table(w); });
    }
    WriteStreamingBlock(
    ...
    while (true) {
        ...
        switch (static_cast<StreamSerializationTag>(block_header.tag)) {
            ...
            case StreamSerializationTag::SOURCE_ID_TABLE:
                ReadSeekableBlockPayload(block_reader, block_header, [this](StreamReader& block) {
                    this->deserialize_source_id_table(block);
                });
                break;
...
Pyramid::Deserialize(StreamReader& reader) {
    ...
    auto max_capacity = basic_info["max_capacity"].GetInt();
    auto param_json = JsonType::Parse(basic_info[INDEX_PARAM].GetString());
    this->persist_source_id_ = param_json.Contains(PYRAMID_PERSIST_SOURCE_ID_KEY) &&
                               param_json[PYRAMID_PERSIST_SOURCE_ID_KEY].GetBool();
    ...
    if (this->persist_source_id_) {
        this->deserialize_source_id_table(buffer_reader);
    }
    ...
    int64_t local_cur_element_count = 0;
    {
        ...
        for (int64_t i = 0; i < data_num; ++i) {
            if (not label_table_->CheckLabel(data_ids[i])) {
                ...
                if (src_ids != nullptr) {
                    label_table_->InsertSourceId(inner_id, src_ids[i]);
                }
...
Pyramid::ExportCache(std::ostream& out_stream) const {
    ...
    WriteStreamingBlock(
        writer,
        cache_tag,
        StreamSerializationTagCritical(cache_tag),
        [&cache_snapshot](StreamWriter& block) { cache_snapshot.Serialize(block); });
...
Pyramid::ImportCache(std::istream& in_stream) {
    ...
    while (true) {
        ...
        if (!StreamSerializationBlockVersionSupported(block_header.tag,
            ...
            if (block_header.IsCritical()) {
                throw VsagException(ErrorType::UNSUPPORTED_INDEX_OPERATION,
                                    "unsupported Pyramid build cache block version");
            }
            block_reader.SkipRemaining();
            continue;
        }
        switch (static_cast<StreamSerializationTag>(block_header.tag)) {
            ...
            default:
                if (block_header.IsCritical()) {
                    throw VsagException(ErrorType::UNSUPPORTED_INDEX_OPERATION,
                                        "unknown Pyramid build cache block");
                }
                break;
        }
        block_reader.SkipRemaining();
    ...
    if (!loaded_cache) {
        throw VsagException(ErrorType::READ_ERROR, "Pyramid build cache block is missing");
    }
}
...
Pyramid::build_with_cache(const DatasetPtr& base) {
    ...
    for (const auto& [hname, h_ptr] : hierarchies_) {
        ...
        for (const auto& [node_path, gnode] : graph_nodes) {
            ...
            // Match HGraph's cache-hit phase: use a self entry point and one-third of the
            // construction budget. A zero budget falls back to full construction EF in
            // add_one_point(), just as HGraph's refine_ef=0 does.
            refine_nodes(node_hit_ids, h_ptr->ef_construction / 3, true);
