...
Pyramid::collect_streaming_header() const {
    ...
    AppendStreamingManifestBlock(manifest, base_tag, 1, StreamSerializationTagCritical(base_tag));
    ...
    AppendStreamingManifestBlock(
        manifest, hierarchy_tag, 1, StreamSerializationTagCritical(hierarchy_tag));
    ...
    while (true) {
        ...
        switch (static_cast<StreamSerializationTag>(block_header.tag)) {
            ...
            case StreamSerializationTag::BASE_CODES:
                this->base_codes_->Deserialize(block_reader);
            ...
            case StreamSerializationTag::HIGH_PRECISION_CODES:
                if (this->use_reorder_) {
                    this->precise_codes_->Deserialize(block_reader);
                    loaded_precise_codes = true;
                }
                break;
            case StreamSerializationTag::PYRAMID_HIERARCHIES:
                this->deserialize_hierarchies(block_reader, basic_info);
