...
BruteForce::parse_streaming_load_parameters(const std::string& parameters) {
    StreamingLoadPlan plan;
    auto json = JsonType::Parse(parameters.empty() ? "{}" : parameters);
    ...
    if (json.Contains(USE_ATTRIBUTE_FILTER) && !json[USE_ATTRIBUTE_FILTER].GetBool()) {
    ...
    while (true) {
        ...
        switch (static_cast<StreamSerializationTag>(block_header.tag)) {
            ...
            case StreamSerializationTag::BASE_CODES:
                ...
                this->inner_codes_->Deserialize(block_reader);
