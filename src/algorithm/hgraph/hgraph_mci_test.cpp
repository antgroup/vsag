...
TEST_CASE("MCI full build covers duplicate vectors after alpha exhaustion",
    ...
    for (const auto& clique : cliques) {
        ...
        REQUIRE(clique.size() <= params.candidate_limit + 1);
        for (auto id : clique) {
            REQUIRE(id < total);
            covered[id] = true;
        }
...
TEST_CASE("MCI shared builder separates enumeration threshold from storage cap",
          "[ut][hgraph][mci][shared_build]") {
    vsag::DefaultAllocator allocator;
    vsag::MCIV3BuildParams params;
    params.total = 6;
    params.candidate_limit = 5;
    params.clique_max = 3;
    const bool incremental = GENERATE(false, true);
    if (incremental) {
        params.clique_size_cap = 3;
    }
    vsag::MCILocalCliqueBuilder builder(params, &allocator);
    ...
    coverage[5].store(0);
    const vsag::InnerIdType neighbors[]{0, 1, 2, 3, 4};
    auto distance = [](auto, auto) { return 1.0F; };
    auto batch = [](auto, auto, auto, auto, auto, float* values) { std::fill_n(values, 4, 1.0F); };
    std::vector<vsag::InnerIdType> stored;
    builder.Build(5, neighbors, 5, 2.0F, coverage, distance, batch, [&](const auto& clique) {
        stored.assign(clique.begin(), clique.end());
    });
    REQUIRE(stored.size() == (incremental ? 3 : 6));
    REQUIRE(std::find(stored.begin(), stored.end(), 5) != stored.end());
    for (uint64_t id = 0; id < coverage.size(); ++id) {
        const bool present = std::find(stored.begin(), stored.end(), id) != stored.end();
        REQUIRE(coverage[id].load() == (id == 5 ? 0 : 1) + static_cast<int>(present));
    }
}

TEST_CASE("HGraph MCI bitmap remains equivalent after removal readdition and reload",
          "[ut][hgraph][mci][bitmap_mutation]") {
    constexpr int64_t dim = 4;
    constexpr int64_t total = 64;
    std::vector<int64_t> ids(total);
    std::iota(ids.begin(), ids.end(), 0);
    ...
    for (int64_t i = 0; i < total; ++i) {
        ...
        vectors[i * dim + 2] = static_cast<float>(i % 11);
        vectors[i * dim + 3] = static_cast<float>(i % 13);
    }
    auto config = vsag::JsonType::Parse(generate_hgraph_mci_params(dim));
    const std::string io = GENERATE("memory_io", "block_memory_io");
    config["index_param"]["base_io_type"].SetString(io);
    config["index_param"]["support_force_remove"].SetBool(true);
    config["index_param"]["build_thread_count"].SetInt(1);
    auto index = vsag::Factory::CreateIndex("hgraph", config.Dump()).value();
    REQUIRE(index->Build(make_dataset(ids, vectors, 0, total, dim)).has_value());
    std::vector<int64_t> valid_ids(total / 2);
    std::iota(valid_ids.begin(), valid_ids.end(), 0);
    // Cover the extra physical slot left by MARK_REMOVE followed by ADD.
    auto bitmap = std::make_shared<RangeBitmapFilter>(0, total + 1, total / 2, valid_ids, true);
    auto callback = std::make_shared<RangeBitmapFilter>(0, total + 1, total / 2, valid_ids, false);
    const std::string search = R"({"hgraph":{"ef_search":100,"use_mci":true,"mci_seed_ratio":10,)"
                               R"("hgraph_valid_ratio_threshold":1}})";
    auto verify = [&](bool fast_path, bool removed) {
        auto query = make_dataset(ids, vectors, 0, 1, dim);
        auto fast = index->KnnSearch(query, total, search, bitmap);
        auto slow = index->KnnSearch(query, total, search, callback);
        REQUIRE(fast.has_value());
        REQUIRE(slow.has_value());
        REQUIRE(fast.value()->GetStatistics({"mci_hybrid_route"})[0] == R"("mci")");
        REQUIRE(vsag::JsonType::Parse(fast.value()->GetStatistics())["mci_bitmap_fast_path"]
                    .GetBool() == fast_path);
        REQUIRE(fast.value()->GetDim() == total / 2 - static_cast<int64_t>(removed));
        REQUIRE(fast.value()->GetDim() == slow.value()->GetDim());
        for (int64_t i = 0; i < fast.value()->GetDim(); ++i) {
            const auto id = fast.value()->GetIds()[i];
            REQUIRE(id == slow.value()->GetIds()[i]);
            REQUIRE(fast.value()->GetDistances()[i] == slow.value()->GetDistances()[i]);
            REQUIRE(id >= 0);
            REQUIRE(id < total / 2);
            if (removed) {
                REQUIRE(id != 0);
    ...
    verify(true, false);  // Warm the identity-mapping cache before changing labels.
    const auto mode = GENERATE(vsag::RemoveMode::MARK_REMOVE, vsag::RemoveMode::FORCE_REMOVE);
    REQUIRE(index->Remove({0}, mode).value() == 1);
    const bool identity_after_remove = mode == vsag::RemoveMode::MARK_REMOVE;
    verify(identity_after_remove, true);
    reload();
    verify(identity_after_remove, true);
    REQUIRE(index->Add(make_dataset(ids, vectors, 0, 1, dim)).value().empty());
    verify(false, false);
    reload();
    verify(false, false);
}
