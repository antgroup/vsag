...
SINDI::UseTermListsHeapInsert(const SINDISearchParameter& search_param) const {
    // Low-prune searches keep the old distance-array heap insertion path for accuracy.
    // Once either side prunes more aggressively, term-list heap insertion avoids scanning
    // the whole window for heap updates.
