...
constexpr uint32_t K_SPARSE_DMQ_BACKEND_MAGIC = 0x53444D51U;
constexpr uint32_t K_SPARSE_DMQ_BACKEND_VERSION = 4;
constexpr uint64_t K_MAX_DMQ_QUERY_LOOKUP_VALUES = 1'000'000;
constexpr uint32_t K_INVALID_CODEBOOK_INDEX = std::numeric_limits<uint32_t>::max();
constexpr uint32_t K_DMQ_DECODE_BLOCK_SIZE = 32;
...
public:
    SINDIDmqRerankQueryContext(Vector<uint32_t> sorted_ids,
        ...
        if (lookup_size > K_MAX_DMQ_QUERY_LOOKUP_VALUES) {
            return;
        }
...
SINDIDmqRerankBackend::LoadIdBlock(uint64_t term_offset,
    ...
    auto skip_bits = static_cast<uint32_t>(bit_offset % 8);
...
SINDIDmqRerankBackend::GetCodebookIndex(uint32_t term_id) const {
    if (term_id < codebook_index_lookup_.size()) {
        ...
        if (index != K_INVALID_CODEBOOK_INDEX) {
            return index;
        }
    }
...
SINDIDmqRerankBackend::RebuildCodebookLookup() {
    ...
    for (uint32_t term_id : codebook_term_ids_) {
        if (term_id < K_MAX_DMQ_QUERY_LOOKUP_VALUES) {
            lookup_size = std::max<uint32_t>(lookup_size, term_id + 1);
        }
    }

    codebook_index_lookup_.assign(lookup_size, K_INVALID_CODEBOOK_INDEX);
...
SINDIDmqRerankBackend::TrainCodebooks(const DatasetPtr& base, bool train_missing_only) {
    ...
    for (int64_t vector_index = 0; vector_index < data_num; ++vector_index) {
        ...
        means[vector_index] = vector.len_ > 0 ? static_cast<float>(value_sum / vector.len_) : 0.0F;
        ...
        for (uint32_t term_index = 0; term_index < vector.len_; ++term_index) {
            ...
            if (iterator == local_term_indexes.end()) {
                auto local_index = static_cast<uint32_t>(local_term_ids.size());
...
SINDIDmqRerankBackend::Add(const DatasetPtr& base) {
    ...
    for (int64_t i = 0; i < data_num; ++i) {
        ...
        encoded.factors.mean = encoded.len > 0 ? static_cast<float>(value_sum / encoded.len) : 0.0F;
...
SINDIDmqRerankBackend::CalDistanceByInnerId(const SINDIRerankQueryContext& query_context,
    ...
    const auto& dmq_context = dynamic_cast<const SINDIDmqRerankQueryContext&>(query_context);
    const auto& encoded = encoded_vectors_[inner_id];

    float query_sum = 0.0F;
    float qualifier_product = 0.0F;
    if (dmq_context.has_lookup_) {
        std::array<uint32_t, K_DMQ_DECODE_BLOCK_SIZE> id_block{};
        uint32_t base_offset = 0;
        while (base_offset < encoded.len) {
            uint32_t block_count =
                std::min<uint32_t>(K_DMQ_DECODE_BLOCK_SIZE, encoded.len - base_offset);
...
SINDIDmqRerankBackend::CalDistanceById(const DatasetPtr& query,
    ...
    if (query == nullptr || query->GetNumElements() == 0 || query->GetSparseVectors() == nullptr) {
...
SINDIDmqRerankBackend::GetSparseVectorByInnerId(InnerIdType inner_id,
    ...
    try {
        ...
        if (data->ids_ != nullptr) {
            allocator->Deallocate(data->ids_);
            data->ids_ = nullptr;
        }
        if (data->vals_ != nullptr) {
            allocator->Deallocate(data->vals_);
            data->vals_ = nullptr;
        }
        throw;
    }
...
SINDIDmqRerankBackend::Serialize(StreamWriter& writer) const {
    StreamWriter::WriteObj(writer, K_SPARSE_DMQ_BACKEND_MAGIC);
    StreamWriter::WriteObj(writer, K_SPARSE_DMQ_BACKEND_VERSION);
...
SINDIDmqRerankBackend::Deserialize(StreamReader& reader) {
    ...
    CHECK_ARGUMENT(serialized_magic == K_SPARSE_DMQ_BACKEND_MAGIC,
    ...
    CHECK_ARGUMENT(serialized_version == K_SPARSE_DMQ_BACKEND_VERSION,
    ...
    CHECK_ARGUMENT(cur_element_count_ >= 0,
                   "serialized direct DMQ vector count must not be negative");
    CHECK_ARGUMENT(encoded_vectors_.size() == static_cast<uint64_t>(cur_element_count_),
                   "serialized direct DMQ vector count is inconsistent");
    uint64_t expected_term_offset = 0;
    for (const auto& encoded : encoded_vectors_) {
        CHECK_ARGUMENT(encoded.term_offset == expected_term_offset,
                       "serialized direct DMQ vector metadata is inconsistent");
        CHECK_ARGUMENT(expected_term_offset <= total_term_count_,
                       "serialized direct DMQ vector offset is out of bounds");
        CHECK_ARGUMENT(encoded.len <= total_term_count_ - expected_term_offset,
                       "serialized direct DMQ vector length is out of bounds");
        expected_term_offset += encoded.len;
    }
    CHECK_ARGUMENT(expected_term_offset == total_term_count_,
                   "serialized direct DMQ term count is inconsistent");
    CHECK_ARGUMENT(id_codes_.size() == get_packed_code_size(total_term_count_, id_bits_),
                   "serialized direct DMQ id codes are inconsistent");
    const uint64_t expected_value_code_size =
        total_bits_ == sindi_dmq::kDirectDmqBits
            ? total_term_count_
            : get_packed_code_size(total_term_count_, total_bits_);
    CHECK_ARGUMENT(value_codes_.size() == expected_value_code_size,
                   "serialized direct DMQ value codes are inconsistent");
