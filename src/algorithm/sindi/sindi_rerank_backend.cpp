...
class SINDIFp32RerankQueryContext : public SINDIRerankQueryContext {
public:
    SINDIFp32RerankQueryContext(Vector<uint32_t> sorted_ids, Vector<float> sorted_vals)
        : sorted_ids_(std::move(sorted_ids)), sorted_vals_(std::move(sorted_vals)) {
    }
...
SINDIFp32RerankBackend::SINDIFp32RerankBackend(const IndexCommonParam& common_param) {
    SparseIndexParameterPtr rerank_param = std::make_shared<SparseIndexParameters>();
    rerank_param->need_sort = true;
    sparse_index_ = std::make_shared<SparseIndex>(rerank_param, common_param);
}

void
SINDIFp32RerankBackend::Add(const DatasetPtr& base) {
    sparse_index_->Add(base);
}

std::unique_ptr<SINDIRerankQueryContext>
SINDIFp32RerankBackend::PrepareQuery(const SparseVector& query) const {
    auto [sorted_ids, sorted_vals] = sparse_index_->sort_sparse_vector(query);
    return std::make_unique<SINDIFp32RerankQueryContext>(std::move(sorted_ids),
                                                         std::move(sorted_vals));
}

float
SINDIFp32RerankBackend::CalDistanceByInnerId(const SINDIRerankQueryContext& query_context,
                                             InnerIdType inner_id) const {
    auto& fp32_context = dynamic_cast<const SINDIFp32RerankQueryContext&>(query_context);
    return sparse_index_->CalDistanceByIdUnsafe(
        fp32_context.sorted_ids_, fp32_context.sorted_vals_, inner_id);
}

float
SINDIFp32RerankBackend::CalcDistanceById(const DatasetPtr& vector, int64_t id) const {
    return sparse_index_->CalcDistanceById(vector, id);
}

DatasetPtr
SINDIFp32RerankBackend::CalDistanceById(const DatasetPtr& query,
                                        const int64_t* ids,
                                        int64_t count) const {
    return sparse_index_->CalDistanceById(query, ids, count);
}

void
SINDIFp32RerankBackend::GetSparseVectorByInnerId(InnerIdType inner_id,
                                                 SparseVector* data,
                                                 Allocator* specified_allocator) const {
    sparse_index_->GetSparseVectorByInnerId(inner_id, data, specified_allocator);
}

void
SINDIFp32RerankBackend::Serialize(StreamWriter& writer) const {
    sparse_index_->Serialize(writer);
}

void
SINDIFp32RerankBackend::Deserialize(StreamReader& reader) {
    sparse_index_->Deserialize(reader);
}

int64_t
SINDIFp32RerankBackend::GetMemoryUsage() const {
    return sparse_index_->GetMemoryUsage();
}
