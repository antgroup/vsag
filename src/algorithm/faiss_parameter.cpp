...
FaissParameter::FromJson(const JsonType& json) {
    if (json.contains(FAISS_STRING_KEY)) {
        this->faiss_string = json[FAISS_STRING_KEY];
    }
    if (json.contains(FAISS_INDEX_PATH_KEY)) {
        this->index_path = json[FAISS_INDEX_PATH_KEY];
    }
}
...
FaissParameter::ToJson() const {
    JsonType json;
    json["type"] = INDEX_FAISS;
    json[FAISS_STRING_KEY] = this->faiss_string;
    json[FAISS_INDEX_PATH_KEY] = this->index_path;
    return json;
}
