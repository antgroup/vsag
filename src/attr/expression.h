...
class Expression {
public:
    Expression(ExpressionType expr_type, OpType op_type): expr_type_(expr_type), op_type_(op_type) {
    }
...
static std::string ToString(const ArithmeticOperator& op) {
    switch (op) {
...
static std::string ToString(const LogicalOperator& op) {
    switch (op) {
...
static std::string ToString(const ComparisonOperator& op) {
    switch (op) {
...
inline bool CheckSameVType(const NumericValue&& lhs, const NumericValue&& rhs) {
...
inline NumericValue operator+(const NumericValue& lhs, const NumericValue& rhs) {
    return std::visit([](auto&& l, auto&& r) -> NumericValue {
...
public:
    ...
    explicit FieldExpression(const std::string& name) : Expression(ExpressionType::kFieldExpression,
                                                            OpType::kNone), fieldName(name) {
    }
...
public:
    ...
    explicit NumericConstant(NumericValue value) : Expression(ExpressionType::kNumericConstant,
                                                              OpType::kNone), value(value) {
    }
...
public:
    ...
    explicit StringConstant(const std::string& value) : Expression(ExpressionType::kStringConstant,
                                                            OpType::kNone), value(value) {
    }
...
public:
    ...
    explicit StrListConstant(StrList values) : Expression(ExpressionType::kStrListConstant,
                                                          OpType::kNone),
...
public:
    ...
    explicit IntListConstant(std::vector<long> values) : Expression(
                                                             ExpressionType::kIntListConstant,
                                                             OpType::kNone),
                                                         values(std::move(values)) {
    }
...
public:
    ArithmeticExpression(ExprPtr left, ArithmeticOperator op, ExprPtr right)
        : Expression(ExpressionType::kArithmeticExpression, OpType::kBinary), left(std::move(left)),
...
public:
    ComparisonExpression(ExprPtr left, ComparisonOperator op, ExprPtr right)
        : Expression(ExpressionType::kComparisonExpression, OpType::kBinary), left(std::move(left)),
          op(op), right(std::move(right)) {
    }
...
public:
    IntListExpression(ExprPtr field, const bool is_not_in, IntListConstant::ptr values)
        : Expression(ExpressionType::kIntListExpression, OpType::kBinary),
          field(std::move(field)), is_not_in(is_not_in), values(std::move(values)) {
    }
...
public:
    StrListExpression(ExprPtr field, const bool is_not_in, StrListConstant::ptr values)
        : Expression(ExpressionType::kStrListExpression, OpType::kBinary), field(std::move(field)),
          is_not_in(is_not_in), values(std::move(values)) {
    }
...
public:
    LogicalExpression(ExprPtr left, LogicalOperator op, ExprPtr right)
        : Expression(ExpressionType::kLogicalExpression, OpType::kBinary), left(std::move(left)),
          op(op), right(std::move(right)) {
    }
...
public:
    explicit NotExpression(ExprPtr expr) : Expression(ExpressionType::kNotExpression,
                                                      OpType::kUnary), expr(std::move(expr)) {
    }
