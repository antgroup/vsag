...
get_vectors(DataTypes type,
    ...
    } else if (type == DataTypes::DATA_TYPE_FP16) {
        *vectors_ptr = (void*)base->GetFloat16Vectors();
        *data_size_ptr = dim * sizeof(int16_t);
    } else if (type == DataTypes::DATA_TYPE_BF16) {
        *vectors_ptr = (void*)base->GetBFloat16Vectors();
        *data_size_ptr = dim * sizeof(int16_t);
    } else {
...
set_dataset(DataTypes type,
    ...
    if (type == DataTypes::DATA_TYPE_FLOAT) {
        ...
        base->Float16Vectors((int16_t*)vectors_ptr)->Dim(dim)->Owner(false)->NumElements(num_element);
    } else if (type == DataTypes::DATA_TYPE_BF16) {
        base->BFloat16Vectors((int16_t*)vectors_ptr)->Dim(dim)->Owner(false)->NumElements(num_element);
    } else {
        throw std::invalid_argument(fmt::format("no support for this type: {}", (int)type));
    }
}
