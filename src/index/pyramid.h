...
public:
    Pyramid(PyramidParameters pyramid_param, const IndexCommonParam& commom_param)
        : pyramid_param_(std::move(pyramid_param)),
          ...
          labels_(commom_param_.allocator_.get()) {
        searcher_ = std::make_unique<BasicSearcher>(commom_param_);
        flatten_interface_ptr_ =
            FlattenInterface::MakeInstance(pyramid_param_.flatten_data_cell_param, commom_param_);
        root_ = std::make_shared<IndexNode>(&commom_param_, pyramid_param_.graph_param);
    }
