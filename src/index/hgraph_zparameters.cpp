...
HGraphSearchParameters::FromJson(const std::string& json_string) {
    ...
    obj.ef_search_ = params[INDEX_HGRAPH][HNSW_PARAMETER_EF_RUNTIME];
    CHECK_ARGUMENT((1 <= obj.ef_search_) and (obj.ef_search_ <= 1000),
                   fmt::format("ef_search({}) must in range[1, 1000]", obj.ef_search_));
