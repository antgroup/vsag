...
private:
    ...
    check_valid_bucket_id(BucketIdType bucket_id) {
        if (bucket_id >= this->bucket_counts_ or bucket_id < 0) {
...
BucketDataCell<QuantTmpl, IOTmpl>::BucketDataCell(const QuantizerParamPtr& quantization_param,
    ...
    this->bucket_counts_ = bucket_count;
...
template <typename QuantTmpl, typename IOTmpl>
ComputerInterfacePtr
BucketDataCell<QuantTmpl, IOTmpl>::FactoryComputer(const float* query) {
...
template <typename QuantTmpl, typename IOTmpl>
void
BucketDataCell<QuantTmpl, IOTmpl>::Train(const float* data, uint64_t count) {
    this->quantizer_->Train(data, count);
}

template <typename QuantTmpl, typename IOTmpl>
void
BucketDataCell<QuantTmpl, IOTmpl>::InsertVector(const float* vector,
    ...
    this->insert_vector_with_locate(vector, bucket_id, locate);
}
...
BucketDataCell<QuantTmpl, IOTmpl>::Serialize(StreamWriter& writer) {
    ...
    for (BucketIdType i = 0; i < this->bucket_counts_; ++i) {
