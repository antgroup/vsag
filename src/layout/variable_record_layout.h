...
public:
    ...
    Read(const LocationEntry& location, bool& need_release) const {
        return payload_.Read(
            location_policy_.GetOffset(location), GetRecordLength(location), need_release);
    }
    ...
    MultiRead(const LocationEntry* locations,
        ...
        for (uint64_t i = 0; i < count; ++i) {
            offsets[i] = location_policy_.GetOffset(locations[i]);
            lengths[i] = GetRecordLength(locations[i]);
        }
