...
TransformQuantizer<metric>::TransformQuantizer(const TransformQuantizerParamPtr& param,
    ...
    for (const auto& transform_str : param->tq_chain_) {
        transform_chain_.emplace_back(
            MakeTransformerInstance(transform_str, transformer_param, common_param));
...
TransformQuantizer<metric>::MakeTransformerInstance(std::string transform_str,
    ...
    if (transform_str == TRANSFORMER_TYPE_VALUE_RESIDUAL) {
        return std::make_shared<ResidualTransformer>(common_param, input_dim, output_dim);
    }
    ...
    if (transform_str == TRANSFORMER_TYPE_VALUE_RESIDUAL) {
        return std::make_shared<ResidualTransformer>(common_param, input_dim, output_dim, DEFAULT_CENTROIDS_COUNT);
    }
