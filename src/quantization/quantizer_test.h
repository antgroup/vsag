...
#define DEFINE_QUANTIZER_ENCODE_DECODE_TESTS(TestName, QuantizerT, dims_val, counts_val, error_val, code_max_val) \
    TEST_CASE(TestName " Encode and Decode", "[ut][" TestName "]") { \
        constexpr MetricType metrics[] = {MetricType::METRIC_TYPE_L2SQR, MetricType::METRIC_TYPE_IP}; \
            ...
            for (auto count : counts_val) { \
                for (auto metric : metrics) { \
                    ...
                    if (metric == MetricType::METRIC_TYPE_L2SQR) { \
                        ...
                        TestQuantizerEncodeDecodeSame(quantizer, dim, count, code_max_val, error_val); \
                    } else { \
        ...
        for (auto dim : dims_val) { \
            for (auto count : counts_val) { \
                for (auto metric : metrics) { \
                    auto allocator = SafeAllocator::FactoryDefaultAllocator(); \
                    if (metric == MetricType::METRIC_TYPE_L2SQR) { \
                        QuantizerT<MetricType::METRIC_TYPE_L2SQR> quantizer1(dim, allocator.get()); \
                        QuantizerT<MetricType::METRIC_TYPE_L2SQR> quantizer2(dim, allocator.get()); \
                        TestSerializeAndDeserialize<QuantizerT<MetricType::METRIC_TYPE_L2SQR>, MetricType::METRIC_TYPE_L2SQR>(quantizer1, quantizer2, dim, count, error_val); \
                    } else if (metric == MetricType::METRIC_TYPE_COSINE) { \
                        QuantizerT<MetricType::METRIC_TYPE_COSINE> quantizer1(dim, allocator.get()); \
                        QuantizerT<MetricType::METRIC_TYPE_COSINE> quantizer2(dim, allocator.get()); \
                        TestSerializeAndDeserialize<QuantizerT<MetricType::METRIC_TYPE_COSINE>, MetricType::METRIC_TYPE_COSINE>(quantizer1, quantizer2, dim, count, error_val); \
                    } else { \
                        ...
                        TestSerializeAndDeserialize<QuantizerT<MetricType::METRIC_TYPE_IP>, MetricType::METRIC_TYPE_IP>(quantizer1, quantizer2, dim, count, error_val); \
                    } \
                } \
            } \
        } \
