...
HalfPrecisionQuantizer<Format, metric>::EncodeOneImpl(const DataType* data, uint8_t* codes) const {
    ...
    for (int i = 0; i < this->dim_; ++i) {
        codes_half[i] = Format::FloatToHalf(cur[i]);
    }
    return true;
}
...
template <typename Format, MetricType metric>
float
HalfPrecisionQuantizer<Format, metric>::ComputeImpl(const uint8_t* codes1, const uint8_t* codes2) {
...
HalfPrecisionQuantizer<Format, metric>::ComputeDistImpl(
    ...
    if constexpr (metric == MetricType::METRIC_TYPE_L2SQR) {
        dists[0] = Format::ComputeL2Sqr(buf, codes, this->dim_);
    } else if constexpr (metric == MetricType::METRIC_TYPE_IP or
                         metric == MetricType::METRIC_TYPE_COSINE) {
        dists[0] = 1 - Format::ComputeIP(buf, codes, this->dim_);
    } else {
