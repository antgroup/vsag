...
void
TestQuantizerEncodeDecodeMetricBF16(uint64_t dim,
                                    int count,
                                    float error = 1e-3,
...
TEST_CASE("BF16 Encode and Decode", "[ut][BF16Quantizer]") {
    ...
    for (auto dim : dims) {
        for (auto count : counts) {
            ...
            TestQuantizerEncodeDecodeMetricBF16<metrics[0]>(dim, count, error, error_same);
            TestQuantizerEncodeDecodeMetricBF16<metrics[1]>(dim, count, error, error_same);
        }
    }
}
